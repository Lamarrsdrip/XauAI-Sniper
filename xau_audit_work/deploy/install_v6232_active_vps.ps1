...
$currentEaName = "XAUUSD_AI_Sniper_EA_v6.23.1"
$eaName = "XAUUSD_AI_Sniper_EA_v6.23.2"
$presetName = "XAUUSD_AI_Sniper_EA_v6.23.2_ACTIVE.set"
...
if(!(Test-Path $journalPath) -or !(Select-String -Path $journalPath -SimpleMatch "Exness-MT5Trial9" -Quiet)) {
  throw "Refusing deployment: the current Journal does not prove this is the intended demo account (Exness-MT5Trial9)."
}

$currentEx5 = Join-Path $expertDir ($currentEaName + ".ex5")
...
$rollback = Join-Path $DataRoot ("MQL5\Backups\v6232_active_deploy_" + $stamp)
...
$currentMq5 = Join-Path $expertDir ($currentEaName + ".mq5")
if(Test-Path $currentMq5) { Copy-Item $currentMq5 -Destination $rollback -Force }

$chart = Get-Content $chartPath -Raw -Encoding Unicode
if($chart -notmatch "name=$([regex]::Escape($currentEaName))") {
  throw "Refusing deployment: persisted chart is not attached to $currentEaName."
}
...
$chart = [regex]::Replace($chart,"name=$([regex]::Escape($currentEaName))","name=$eaName",1)
...
$targetEx5 = Join-Path $expertDir ($eaName + ".ex5")
$targetMq5 = Join-Path $expertDir ($eaName + ".mq5")
Copy-Item $stagedEx5 -Destination $targetEx5 -Force
Copy-Item $stagedMq5 -Destination $targetMq5 -Force
Copy-Item $stagedPreset -Destination (Join-Path $presetDir $presetName) -Force

$installedHash = (Get-FileHash $targetEx5 -Algorithm SHA256).Hash
