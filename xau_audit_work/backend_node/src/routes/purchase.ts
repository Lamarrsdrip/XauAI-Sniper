...
const PurchaseInitRequestSchema = z.object({
  ...
  origin_url: z.string().url().max(2048),
  display_currency: z.string().trim().max(12).nullable().optional(),
}).strict();
const BankTransferInitiateRequestSchema = z.object({
  buyer_name: z.string().trim().min(1).max(120),
  buyer_email: z.string().trim().email().max(320),
}).strict();
const BankTransferProofRequestSchema = z.object({ proof_image: z.string().max(7_000_000) }).strict();

const SIGNAL_PLAN_IDS = ["SIGNALS_WEEKLY", "SIGNALS_MONTHLY"] as const;
const SignalPlanInitRequestSchema = z.object({ plan_id: z.enum(SIGNAL_PLAN_IDS), origin_url: z.string().url().max(2048) }).strict();
...
export async function registerPurchaseRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post("/purchase/bank-transfer/:reference/proof", async (request, reply) => {
    ...
    // Restrict stored inline proofs to non-scriptable raster image formats.
    // `data:image/` also accepts SVG, which is active content and should not
    // be persisted into an admin-facing <img> preview.
    if (!/^data:image\/(?:png|jpe?g|webp);base64,[A-Za-z0-9+/=\s]+$/i.test(req.proof_image)) {
      return reply.code(400).send({ detail: "Proof must be a PNG, JPEG, or WebP image." });
    }
    const tx = await getDb().collection("payment_transactions").findOne({ reference, provider: "BANK_TRANSFER" }, { projection: { _id: 0 } });
    if (!tx) return reply.code(404).send({ detail: "Not found" });
