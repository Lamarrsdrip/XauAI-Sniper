...
const AdminSettingsUpdateSchema = z.object({
  paystack_secret_key: z.string().max(1024).nullable().optional(),
  pin_price_kobo: z.number().int().min(0).max(1_000_000_000_000).nullable().optional(),
  smtp_email: z.string().trim().max(320).nullable().optional(),
  smtp_password: z.string().max(1024).nullable().optional(),
  ...
  onesignal_app_id: z.string().trim().max(256).nullable().optional(),
  onesignal_api_key: z.string().max(1024).nullable().optional(),
  email_sender_name: z.string().trim().max(120).nullable().optional(),
  admin_notification_email: z.string().trim().max(320).nullable().optional(),
  support_email: z.string().trim().max(320).nullable().optional(),
  support_phone: z.string().trim().max(64).nullable().optional(),
  community_link: z.string().trim().max(2048).nullable().optional(),
  mt5_download_url: z.string().trim().max(2048).nullable().optional(),
  vps_guide_url: z.string().trim().max(2048).nullable().optional(),
  installation_guide_url: z.string().trim().max(2048).nullable().optional(),
  command_center_url: z.string().trim().max(2048).nullable().optional(),
  signals_weekly_price_kobo: z.number().int().min(0).max(1_000_000_000_000).nullable().optional(),
  signals_monthly_price_kobo: z.number().int().min(0).max(1_000_000_000_000).nullable().optional(),
  subscriber_signal_source_account: z.string().trim().max(64).nullable().optional(),
  subscriber_signal_backup_source_account: z.string().trim().max(64).nullable().optional(),
}).strict();

const NombaConfigUpdateSchema = z.object({
  enabled: z.boolean().nullable().optional(),
  environment: z.enum(["sandbox", "production"]).nullable().optional(),
  sandbox_client_id: z.string().max(1024).nullable().optional(),
  sandbox_client_secret: z.string().max(2048).nullable().optional(),
  sandbox_account_id: z.string().max(1024).nullable().optional(),
  sandbox_webhook_signature_key: z.string().max(2048).nullable().optional(),
  production_client_id: z.string().max(1024).nullable().optional(),
  production_client_secret: z.string().max(2048).nullable().optional(),
  production_account_id: z.string().max(1024).nullable().optional(),
  production_webhook_signature_key: z.string().max(2048).nullable().optional(),
  allowed_payment_methods: z.array(z.string().trim().min(1).max(80)).max(20).nullable().optional(),
  currency: z.string().trim().min(3).max(8).nullable().optional(),
  payment_description: z.string().trim().max(500).nullable().optional(),
  current_password: z.string().max(256).nullable().optional(),
}).strict();
...
const AdminBankTransferSettingsUpdateSchema = z.object({
  ...
  timeout_minutes: z.number().int().min(5).max(10_080).nullable().optional(),
  proof_required: z.boolean().nullable().optional(),
  support_contact: z.string().trim().max(500).nullable().optional(),
...
const AdminPaymentMethodsSettingsUpdateSchema = z.object({
  ...
  default_payment_method: z.string().trim().max(80).nullable().optional(),
  payment_method_order: z.array(z.string().trim().max(80)).max(10).nullable().optional(),
}).strict();
...
export async function registerAdminSettingsRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.put("/admin/settings/payment-methods", { preHandler: requireAdmin }, async (request, reply) => {
    ...
    if (req.payment_method_order != null) {
      ...
      if (new Set(req.payment_method_order).size !== req.payment_method_order.length) {
        return reply.code(400).send({ detail: "payment_method_order cannot contain duplicates." });
      }
