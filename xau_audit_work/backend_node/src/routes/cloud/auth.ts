...
const CloudLoginSchema = z.object({ email: z.string().trim().email().max(320), password: z.string().min(1).max(256) }).strict();
const CloudForgotPasswordSchema = z.object({ email: z.string().trim().email().max(320) }).strict();
const CloudResetPasswordSchema = z.object({ token: z.string().min(1).max(4096), new_password: z.string().min(1).max(256) }).strict();
const CloudDeleteAccountSchema = z.object({ password: z.string().min(1).max(256), confirm: z.boolean().optional().default(false) }).strict();
const CloudLicenseLinkSchema = z.object({ license_key: z.string().trim().min(1).max(128) }).strict();
...
export async function registerCloudAuthRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post("/cloud/account/delete", { preHandler: requireCloudUser }, async (request, reply) => {
    ...
    // requireCloudUser deliberately strips password_hash from request.cloudUser.
    // Re-read the credential hash for this destructive re-authentication step
    // instead of comparing against the (always missing) projected field.
    const db = getDb();
    const credential = await db.collection("cloud_users").findOne(
      { id: user["id"] },
      { projection: { _id: 0, password_hash: 1 } },
    );
    if (!credential || !(await verifyPassword(body.password, String(credential["password_hash"] ?? "")))) {
    ...
    await Promise.all([
      db.collection("cloud_push_subscriptions").deleteMany({ user_id: user["id"] }),
      db.collection("cloud_notification_prefs").deleteMany({ user_id: user["id"] }),
      db.collection("web_push_subscriptions").deleteMany({ user_id: String(user["id"]) }),
      db.collection("cloud_device_tokens").deleteMany({ user_id: String(user["id"]) }),
    ]);
