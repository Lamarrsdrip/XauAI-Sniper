      ...
      if (entry.signature) {
        ...
        try {
          ...
          joinedShadowDoc = await db.collection("ml_shadow_decisions").findOneAndUpdate(
            { signature: entry.signature, account: Number(entry.account_login), actual_action: "CANDIDATE", eventual_result: { $exists: false } },
            {
              $set: {
        ...
        // Global Learning Brain: only authenticated journal fields may enter
        // the trusted corpus. /ml/shadow/record is a legacy unauthenticated EA
        // telemetry endpoint, so even a correctly joined shadow row is
        // diagnostic-only and must never supply regime/setup/hive features to
        // training (otherwise a forged shadow row could poison real outcomes).
        if (isClosedTrade) {
          try {
            await recordGlobalBrainObservation(buildBotTradeObservation(doc, null));
          } catch {
