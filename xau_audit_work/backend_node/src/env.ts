...
function requireEnv(name: string): string {
  ...
  if (value) {
    if (IS_PRODUCTION && name === "JWT_SECRET" && value.length < 32) {
      throw new Error("JWT_SECRET must be at least 32 characters in production. Refusing to sign admin/customer sessions with a weak secret.");
    }
    return value;
  }
  if (IS_PRODUCTION) {
...
export const env = {
  ...
  // Pepper for one-way hashing account/license identity into Global Brain
  // observations' account_ref (services/globalBrainIngest.ts). Falls back to
  // JWT_SECRET (already required in production) when unset, so this never
  // becomes a new fail-closed startup requirement.
  GLOBAL_BRAIN_HASH_PEPPER: process.env["GLOBAL_BRAIN_HASH_PEPPER"] ?? "",
};
