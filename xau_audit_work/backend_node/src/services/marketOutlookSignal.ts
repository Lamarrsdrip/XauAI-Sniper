...
function deriveSetupType(path: string): string {
  if (path.startsWith("PULLBACK") || path.startsWith("RALLY")) return "WITH_TREND_PULLBACK";
  if (path === "DIRECT_CONTINUATION") return "TREND_CONTINUATION";
  if (path === "REVERSAL_FORMING") return "OPPOSITE_DIRECTION_REVERSAL";
  return "NONE";
}
...
export async function generateOutlookForAccount(opts: {
  ...
  let effectiveDirection = directionLabel === "BLOCKED" ? 0 : direction;

  const dataStatus = "LIVE";
  let path = expectedPath(effectiveDirection, thesis);
  let setupType = deriveSetupType(path);

  // Global Learning Brain -- production influence (AGENTS spec sections
  // 19-23). Evaluated AFTER setupType/path so the DIRECTION_QUALITY bucket
  // key (direction|session|regime|setup_type) can include setup_type.
  // Mirrors evaluateOwnerPolicy's own BLOCKED-direction pattern exactly, so
  // a REJECT has the IDENTICAL downstream effect (zone/geometry skipped,
  // primary_direction=BLOCKED, execution_authority false) as every other
  // already-proven block path -- no new blocking mechanism is introduced.
  // Guaranteed no-op while {m10,outlook}_learned_influence_enabled are OFF
  // (their production default): see globalBrainInfluence.ts's safety
  // contract -- this call always returns NO_OPINION/not-applied without
  // touching the registry when the relevant switch is off.
  const globalBrainScope = publicationMode === "M10_SIGNAL" ? "M10" : "OUTLOOK";
  let globalBrainInfluence: GlobalBrainInfluenceResult | null = null;
  let globalBrainBlockedDirection: string | null = null;
  if (["BUY", "SELL"].includes(directionLabel)) {
    globalBrainInfluence = await evaluateGlobalBrainInfluence(globalBrainScope, {
      direction: directionLabel as "BUY" | "SELL",
      session: String(evidence["session"] ?? ""),
      regime: bias.market_regime,
      setup_type: setupType,
    });
    if (["REJECT", "WAIT"].includes(globalBrainInfluence.recommendation)) {
      globalBrainBlockedDirection = directionLabel;
      directionLabel = "BLOCKED";
      effectiveDirection = 0;
      // Re-derive path/setup_type against the now-BLOCKED direction so the
      // published expected_path/narrative never contradicts primary_direction
      // (the owner-policy block above avoids this by running before the
      // original path/setupType computation; this call runs after it).
      path = expectedPath(effectiveDirection, thesis);
      setupType = deriveSetupType(path);
    }
  }
  ...
  if (effectiveDirection !== 0 && ["BUY", "SELL"].includes(directionLabel)) {
    ...
    if (eaStructEntry > 0 && eaStructSl > 0 && eaAtr > 0) {
      ...
      zone = {
        ...
        tp3_r: Math.round(Math.max(1.0, Number.isFinite(Number(thesis["remaining_room_r"])) ? Number(thesis["remaining_room_r"]) : 2.0) * 100) / 100,
      };
    } else {
  ...
  const doc: Record<string, unknown> = {
    ...
    global_brain_influence: globalBrainInfluence,
    global_brain_blocked_direction: globalBrainBlockedDirection,
    expected_path: path,
  ...
  const inserted = await insertOutlookAtomically(doc, account, OUTLOOK_SYMBOL, hourlySlot, publicationKey);
  // Global Learning Brain: real-time shadow comparison log (rule decision
  // vs. current champion's suggestion), strictly AFTER the real Outlook
  // decision above is already durably persisted. logOutlookShadowComparison
  // self-catches internally and never throws -- awaiting it here only
  // guarantees the log write completes before this function returns, it
  // cannot affect `inserted` (already computed) or fail this call.
  await logOutlookShadowComparison(doc);
  return inserted;
}
