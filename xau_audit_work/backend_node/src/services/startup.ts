...
export async function runStartupTasks(log: FastifyBaseLogger): Promise<void> {
  ...
  if (!anyAdmin) {
    if (!adminPasswordEnv && IS_PRODUCTION) {
      throw new Error("ADMIN_PASSWORD is required to seed the first production admin. Refusing to generate and print a production password in logs.");
    }
  ...
  // ADMIN_PASSWORD is a bootstrap credential, not a perpetual source of truth.
  // Once an admin exists, Account settings own the password. Re-syncing the
  // environment value on every restart silently undid legitimate password
  // changes and made the in-product password-change flow unreliable.
  ...
  const lifecycleIndexes: Array<[string, () => Promise<unknown>]> = [
    ["cloud_market_outlooks.id", () => db.collection("cloud_market_outlooks").createIndex("id", { unique: true })],
    ["cloud_market_outlooks.account+generated_at", () => db.collection("cloud_market_outlooks").createIndex({ account: 1, generated_at: -1 })],
    ["cloud_market_outlooks.lifecycle", () => db.collection("cloud_market_outlooks").createIndex({ monitoring_closed: 1, primary_direction: 1, account: 1 })],
    ["cloud_market_outlook_outcomes.outlook_id", () => db.collection("cloud_market_outlook_outcomes").createIndex("outlook_id", { unique: true })],
    ["cloud_bot_activity.account+ts", () => db.collection("cloud_bot_activity").createIndex({ account: 1, ts: 1 })],
    ["cloud_bot_activity.symbol+ts", () => db.collection("cloud_bot_activity").createIndex({ normalized_symbol: 1, ts: -1 })],
    ["cloud_bot_activity.account+symbol+ts", () => db.collection("cloud_bot_activity").createIndex({ account: 1, normalized_symbol: 1, ts: 1 })],
    ["manual_trading_broker_candles.identity", () => db.collection("manual_trading_broker_candles").createIndex({ account: 1, symbol: 1, timeframe: 1, openTime: 1 }, { unique: true })],
    ["manual_trading_broker_candles.lastSourceAt", () => db.collection("manual_trading_broker_candles").createIndex({ lastSourceAt: -1 })],
    ["manual_trading_ea_decisions.account+symbol+recordedAt", () => db.collection("manual_trading_ea_decisions").createIndex({ account: 1, symbol: 1, recordedAt: -1 })],
    ["manual_trading_ea_decisions.ttl", () => db.collection("manual_trading_ea_decisions").createIndex("recordedAt", { expireAfterSeconds: 180 * 86400 })],
    ["cloud_notification_prefs.user_id", () => db.collection("cloud_notification_prefs").createIndex("user_id", { unique: true })],
    ["cloud_push_subscriptions.user+opted", () => db.collection("cloud_push_subscriptions").createIndex({ user_id: 1, opted_in: 1 })],
    ["cloud_outlook_signal_events.identity", () => db.collection("cloud_outlook_signal_events").createIndex({ account: 1, candidate_id: 1, event_type: 1, event_version: 1 }, { unique: true })],
    ["cloud_outlook_signal_events.timeline", () => db.collection("cloud_outlook_signal_events").createIndex({ account: 1, symbol: 1, signal_bar_time: -1, event_time: -1 })],
  ];
  for (const [label, create] of lifecycleIndexes) {
    await create().catch((e) => log.warn(`[signal-outlook] index ${label} failed: ${String(e)}`));
  }
