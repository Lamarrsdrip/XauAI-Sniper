...
import { ensureGlobalBrainShadowServingIndexes } from "./services/globalBrainShadowServing.js";
import { checkExpoPushReceipts, ensurePushReceiptIndexes } from "./services/expoPush.js";
...
import { registerAcademyCourseRoutes } from "./routes/cloud/academyCourses.js";
...
async function main(): Promise<void> {
  ...
  await app.register(
    async (api) => {
      ...
      await registerAdminGlobalBrainRoutes(api);
      ...
      await registerAcademyCourseRoutes(api);
  ...
  // Global Learning Brain daily cycle -- fires once at a fixed UTC hour
  // (03:00, a low-traffic window) rather than a flat 24h-from-start
  // interval, matching outlookHourlyLoop's real-boundary-alignment
  // convention. A failed cycle (caught inside runGlobalBrainDailyCycle
  // itself, per the DAILY JOB FAILURE rule) never crashes this loop and
  // never blocks tomorrow's attempt.
  const GLOBAL_BRAIN_CYCLE_HOUR_UTC = 3;
  async function globalBrainDailyLoop(): Promise<void> {
    for (;;) {
      const now = new Date();
      const next = new Date(now);
      next.setUTCHours(GLOBAL_BRAIN_CYCLE_HOUR_UTC, 0, 0, 0);
      if (next.getTime() <= now.getTime()) next.setUTCDate(next.getUTCDate() + 1);
      const sleepSeconds = Math.max(60, (next.getTime() - now.getTime()) / 1000);
      await new Promise((resolve) => setTimeout(resolve, sleepSeconds * 1000));
      try {
        const settings = await getGlobalBrainSettings();
        if (!settings.scheduled_cycle_enabled) {
          app.log.info("[global-brain] scheduled cycle skipped -- scheduled_cycle_enabled is OFF (manual /admin/global-brain/run-cycle still works)");
          continue;
        }
        const report = await runGlobalBrainDailyCycle();
        app.log.info({ success: report.success, promoted: Object.values(report.questions).filter((q) => q?.promoted).length }, "[global-brain] daily cycle complete");
      } catch (e) {
        app.log.warn(`[global-brain] daily cycle failed: ${String(e)}`);
      }
    }
  }
  ...
  // Forensic-incident fix: "SENT" in cloud_notification_log previously only
  // ever meant Expo's send API accepted the request, never a delivery
  // confirmation -- this periodically asks Expo what actually happened to
  // recently-sent tickets, and prunes any device token a receipt reveals is
  // permanently dead (DeviceNotRegistered/InvalidCredentials).
  setInterval(() => { void checkExpoPushReceipts().catch((error) => app.log.warn({ error }, "[push-receipts] check failed")); }, 15 * 60_000).unref();
  ...
  // Historical repair is deliberately off the readiness critical path. It is
  // bounded and idempotent, and never invents outcomes from sparse history.
  void backfillSignalOutlookHistory()
    .then((report) => app.log.info({ report }, "[outlook-history-repair] complete"))
    .catch((error) => app.log.warn({ error }, "[outlook-history-repair] failed"));
