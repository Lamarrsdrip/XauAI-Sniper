...
#property version   "6.1.4"
#property description "XAUUSD AI Sniper v6.1.4 — HTF PULLBACK BUY: buy M5 dips in confirmed H1+HTF bull trend"
...
int ScoreSetups(double &score, string &setupName)
{
   ...
   if(InpAllowTrendPullback &&
      (currentRegime == REGIME_TRENDING_UP || currentRegime == REGIME_TRENDING_DOWN ||
       (htfBullConsensus && (currentRegime == REGIME_RANGING || currentRegime == REGIME_LOW_VOL)) ||
       (htfBearConsensus && (currentRegime == REGIME_RANGING || currentRegime == REGIME_LOW_VOL))))
   {
      double s = 0;
