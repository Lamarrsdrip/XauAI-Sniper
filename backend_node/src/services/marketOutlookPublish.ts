...
export async function publishM10SignalFromActivity(licenseKey: string, account: string, sourceEventId = ""): Promise<Record<string, unknown> | null> {
  ...
  // Global Learning Brain: a BLOCKED/EXPIRED M10 candidate never becomes an
  // actionable, outcome-tracked signal, so it must be captured here or not
  // at all (see globalBrainIngest.ts's buildM10CandidateObservation).
  // Best-effort, strictly after the real lifecycle event above is already
  // durably persisted. Explicitly try/catch'd (an adversarial review found
  // this previously had none): buildM10CandidateObservation is a
  // synchronous pure function that could throw on malformed eventDoc data,
  // and this line runs BEFORE the actionable-signal generation/dispatch
  // code below -- an uncaught throw here would have silently skipped
  // real M10 signal generation for this account, not just the logging.
  try {
