...
function renderBlock(block: EmailBlock, theme: EmailDocument["theme"], recipient: Personalization): string {
  ...
  switch (block.type) {
    case "hero":
      return `<tr><td class="xc-content-pad" style="padding:42px 38px 38px;background:#111114;text-align:${block.align ?? "left"};"><div style="margin-bottom:12px;color:${theme.accent};font:700 10px/14px Arial,sans-serif;letter-spacing:1.8px;text-transform:uppercase;">${esc(block.badge || "XauCloud update")}</div><div style="color:#FFFFFF;font:800 32px/39px Arial,sans-serif;letter-spacing:-.8px;">${esc(title)}</div>${subtitle ? `<div style="margin-top:14px;color:#B8B8C0;font:400 16px/25px Arial,sans-serif;">${esc(subtitle)}</div>` : ""}${block.url && text ? `<div style="margin-top:26px;">${button(text, block.url, block, theme.accent)}</div>` : ""}</td></tr>`;
    case "announcement":
    ...
    case "metrics": {
      ...
      const cells = items.map((item) => `<td class="xc-stack" width="${100 / Math.max(items.length, 1)}%" valign="top" style="padding:6px;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td style="padding:18px 12px;background:#111114;border-top:2px solid ${theme.accent};text-align:center;"><div style="color:#FFFFFF;font:800 23px/29px Arial,sans-serif;">${esc(personalize(item.value || "", recipient))}</div><div style="margin-top:5px;color:#A9A9B1;font:700 9px/13px Arial,sans-serif;letter-spacing:1px;text-transform:uppercase;">${esc(personalize(item.label, recipient))}</div></td></tr></table></td>`).join("");
    ...
    case "feature":
      return `<tr><td class="xc-content-pad" style="padding:${pad};background:${block.background ?? theme.contentBackground};"><table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td width="40" valign="top"><div style="width:30px;height:30px;background:#111114;color:${theme.accent};font:800 14px/30px Arial,sans-serif;text-align:center;">${esc(block.badge || "✓")}</div></td><td valign="top"><div style="color:#111114;font:800 16px/22px Arial,sans-serif;">${esc(title)}</div><div style="margin-top:5px;color:#55555D;font:400 14px/22px Arial,sans-serif;">${rich(block.html || text, recipient)}</div></td></tr></table></td></tr>`;
    case "steps": {
