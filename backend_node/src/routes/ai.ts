...
import { applyGlobalBrainToEntryVerdict, evaluateGlobalBrainInfluence, type GlobalBrainInfluenceResult } from "../services/globalBrainInfluence.js";
...
Decision (HOLD / CLOSE / LOCK)? JSON only.`;
  ...
  app.post("/ai/analyze", async (request, reply) => {
    ...
    try {
      ...
      // 19-23). Consulted only after the existing dual-AI verdict is
      // finalized, and may only ever DOWNGRADE an already-decided BUY/SELL
      // to SKIP -- it can never invent a trade this endpoint's own logic
      // did not already independently produce, and it never touches
      // sl_adjust/tp_adjust/confidence beyond the downgrade itself (spec
      // section 19: no risk/sizing/safety authority). Guaranteed no-op
      // while bot_learned_influence_enabled is OFF (production default):
      // see globalBrainInfluence.ts's safety contract.
      let globalBrainInfluence: GlobalBrainInfluenceResult | null = null;
      if (["BUY", "SELL"].includes(action)) {
        globalBrainInfluence = await evaluateGlobalBrainInfluence("BOT", {
          direction: action as "BUY" | "SELL",
          session: req.session,
          regime: req.regime,
          setup_type: req.setup,
        });
        const adjusted = applyGlobalBrainToEntryVerdict({ action, confidence, reason, sl_adjust: slAdj, tp_adjust: tpAdj }, globalBrainInfluence);
        action = adjusted.action;
        confidence = adjusted.confidence;
        reason = adjusted.reason;
        slAdj = adjusted.sl_adjust;
        tpAdj = adjusted.tp_adjust;
      }
