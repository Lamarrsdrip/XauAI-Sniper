...
const DiagnosticsOperation = z.enum(["admin_overview","system_health","service_status","integration_health","payment_provider_status","recent_errors","recent_warnings","audit_search","audit_event","dashboard_metrics","dashboard_metrics_range","compare_dashboard_periods","conversion_funnel","revenue_summary","license_summary","support_summary"]);
...
/** One authoritative, read-only operational snapshot.  Missing telemetry is
 * reported as unavailable rather than guessed. */
async function adminOverview(): Promise<Record<string, unknown>> {
  const db = getDb(); const now = new Date(); const start = new Date(now); start.setUTCHours(0, 0, 0, 0);
  const yesterday = new Date(start); yesterday.setUTCDate(yesterday.getUTCDate() - 1);
  const nowIso = now.toISOString(); const startIso = start.toISOString(); const yesterdayIso = yesterday.toISOString(); const startEpoch = Math.floor(start.getTime() / 1000);
  const heartbeatRows = await db.collection("cloud_bot_heartbeats").aggregate([{ $sort: { ts: -1 } }, { $group: { _id: { $ifNull: ["$account_number", "$account_login"] }, latest: { $first: "$$ROOT" } } }]).toArray();
  const staleBefore = new Date(now.getTime() - 10 * 60_000).toISOString();
  const [activeUsers, registrationsToday, registrationsPreviousDay, activeLicenses, closedToday, dailyPnl, paidOrders, failedPayments, bankSubmitted, abandoned, openTickets, emailFailures, xSettings, xFailed, outlook, errors, warnings] = await Promise.all([
    db.collection("cloud_users").countDocuments({}), db.collection("cloud_users").countDocuments({ created_at: { $gte: startIso } }), db.collection("cloud_users").countDocuments({ created_at: { $gte: yesterdayIso, $lt: startIso } }), db.collection("pin_licenses").countDocuments({ is_active: true }),
    db.collection("trade_journal").countDocuments({ has_rich_ledger_data: true, closed_at: { $gte: startEpoch } }), db.collection("trade_journal").aggregate([{ $match: { has_rich_ledger_data: true, closed_at: { $gte: startEpoch } } }, { $group: { _id: null, net_usd: { $sum: "$profit" }, wins: { $sum: { $cond: [{ $gt: ["$profit", 0] }, 1, 0] } }, losses: { $sum: { $cond: [{ $lt: ["$profit", 0] }, 1, 0] } }, breakevens: { $sum: { $cond: [{ $eq: ["$profit", 0] }, 1, 0] } } } }]).toArray(),
    db.collection("payment_transactions").countDocuments({ payment_status: { $in: ["SUCCESS", "FULFILLED", "PAID", "success"] } }), db.collection("payment_transactions").countDocuments({ payment_status: { $in: ["FAILED", "ERROR"] } }), db.collection("payment_transactions").countDocuments({ provider: "BANK_TRANSFER", payment_status: { $in: ["SUBMITTED", "PENDING"] } }), db.collection("payment_transactions").countDocuments({ payment_status: { $nin: ["SUCCESS", "FULFILLED", "PAID", "success"] }, pin_generated: { $in: [null, "", false] } }),
    db.collection("support_tickets").countDocuments({ status: { $ne: "closed" } }), db.collection("admin_email_log").countDocuments({ failed: { $gt: 0 } }), xPostingSettings(), db.collection("x_trade_posts").countDocuments({ status: "failed" }), db.collection("cloud_market_outlooks").findOne({}, { projection: { _id: 0, id: 1, generated_at: 1, status: 1, primary_direction: 1 }, sort: { generated_at: -1 } }), Promise.resolve(recentDiagnostics("error", 20)), Promise.resolve(recentDiagnostics("warning", 20)),
  ]);
  const latest = heartbeatRows.map((row) => row["latest"] as Record<string, unknown>).filter(Boolean);
  const liveBots = latest.filter((row) => String(row["ts"] ?? "") >= staleBefore);
  const staleBots = latest.filter((row) => String(row["ts"] ?? "") < staleBefore);
  const daily = dailyPnl[0] ?? {};
  return { generated_at: nowIso, production: { readiness: readinessSnapshot().state, release: await releaseSummary() }, users: { active: activeUsers, registrations_today: registrationsToday, registrations_previous_day: registrationsPreviousDay, registrations_change: registrationsToday - registrationsPreviousDay }, licenses: { active: activeLicenses }, bots: { connected_accounts: latest.length, live: liveBots.length, stale: staleBots.length, disconnected: null, disconnected_reason: "No authoritative disconnect event is persisted; stale heartbeat count is reported instead.", open_positions: latest.reduce((sum, row) => sum + Number(row["open_positions"] ?? 0), 0) }, trading: { closed_today: closedToday, wins_today: Number(daily["wins"] ?? 0), losses_today: Number(daily["losses"] ?? 0), breakevens_today: Number(daily["breakevens"] ?? 0), realized_net_usd: Number(daily["net_usd"] ?? 0) }, outlook: { latest: outlook ?? null, available: Boolean(outlook) }, orders: { paid: paidOrders, failed_payments: failedPayments, bank_transfer_submitted: bankSubmitted, abandoned_checkout: abandoned }, support: { requiring_attention: openTickets }, email: { delivery_failures: emailFailures }, x_posting: { auto_post_enabled: xSettings["auto_post_enabled"], failed_posts: xFailed }, diagnostics: { recent_errors: errors, recent_warnings: warnings } };
}
