...
export async function registerMarketingActionRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post("/admin/actions/marketing/campaigns/:campaignId/announcements/:assetId/publish", action, async (request, reply) => { try { const p = AssetParamsSchema.parse(request.params); const req = ConfirmSchema.parse(request.body ?? {}); return await publishVersionedAsset("announcement", "marketing_announcements", p.campaignId, p.assetId, "publish", req.confirmation_token, req.idempotency_key); } catch (error) { return fail(reply, error); } });
  ...
  app.post("/admin/actions/marketing/campaigns/:campaignId/landing-pages/:assetId/publish", action, async (request, reply) => { try { const p = AssetParamsSchema.parse(request.params); const req = ConfirmSchema.parse(request.body ?? {}); return await publishVersionedAsset("landing", "marketing_landing_pages", p.campaignId, p.assetId, "publish", req.confirmation_token, req.idempotency_key); } catch (error) { return fail(reply, error); } });
