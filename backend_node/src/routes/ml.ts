...
export async function registerMlRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post("/ml/shadow/record", async (request, reply) => {
    ...
    try {
      ...
      // Global Learning Brain: a SKIPPED decision (rule engine rejected this
      // candidate) never gets a later trade_journal close event to join
      // against, so it must be captured here or not at all. Own try/catch
      // (an adversarial review found this previously relied on the outer
      // catch below, which would flip this response to {status:"error"}
      // even though the real insertOne just above already succeeded --
      // misleading to the EA and could prompt a duplicate retry insert).
      try {
        if (req.actual_action === "SKIPPED") {
          await recordGlobalBrainObservation(buildShadowCandidateObservation(req));
        }
        // Real-time shadow serving: log what the current champion would
        // suggest for this exact decision, alongside the rule/hive decision
