...
#property version   "6.1.3"
#property description "XAUUSD AI Sniper v6.1.3 — ANTI-TREND PROTECTION + BASKET LOSS BLOCK"
...
int ScoreSetups(double &score, string &setupName)
{
   ...
   // signals regardless of which setup generated them. TREND_PULLBACK (type 1) and
   // BREAKOUT (type 3) and ASIA_BREAKOUT (type 8) are exempted — they already require
   // regime alignment. All other setups are reversal/fade types that must not fight
   // a confirmed multi-TF trend.
   if(bestDir == -1 && htfBullConsensus && bestType != 1 && bestType != 3 && bestType != 8)
   {
   ...
   if(bestDir == 1 && htfBearConsensus && bestType != 1 && bestType != 3 && bestType != 8)
   {
      Print("ANTI-TREND VETO: H1+HTF both BEAR — BUY blocked | setup=", bestName,
            " h1Spread=", DoubleToString(h1Spread*100, 3), "% htfSpread=", DoubleToString(htfSpread*100, 3), "%");
      bestDir = 0; bestScore = 0; bestName = "";
   }
   // Even for TREND_PULLBACK: if H1+HTF both bullish, no sells (pullback sells against
   // consensus are the specific pattern causing the -$260 losses reported).
   if(bestDir == -1 && htfBullConsensus && bestType == 1)
   {
      Print("ANTI-TREND VETO: H1+HTF both BULL — TREND_PULLBACK SELL also blocked (M5 pullback in bull trend)",
            " h1Spread=", DoubleToString(h1Spread*100, 3), "% htfSpread=", DoubleToString(htfSpread*100, 3), "%");
      bestDir = 0; bestScore = 0; bestName = "";
   }
   if(bestDir == 1 && htfBearConsensus && bestType == 1)
   {
      Print("ANTI-TREND VETO: H1+HTF both BEAR — TREND_PULLBACK BUY also blocked",
            " h1Spread=", DoubleToString(h1Spread*100, 3), "% htfSpread=", DoubleToString(htfSpread*100, 3), "%");
      bestDir = 0; bestScore = 0; bestName = "";
   }
