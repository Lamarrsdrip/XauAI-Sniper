...
import { CurrencyBtc, ShoppingCart, ShieldCheck, Lightning, Check } from "@phosphor-icons/react";
...
export default function PurchaseSection({ api }) {
  ...
  useEffect(() => {
    axios.get(`${api}/purchase/price`).then(r => setPrice(r.data.price)).catch(() => {});
  }, [api]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/10 border border-white/20 mb-6">
              <CurrencyBtc size={12} weight="bold" className="text-[hsl(43,74%,49%)]" />
              <span className="text-xs font-mono font-medium tracking-[0.15em] text-white/70">
                BUY WITH CRYPTO
              </span>
            </div>
            <h2 className="font-heading text-3xl sm:text-4xl font-black tracking-tight mb-4" data-testid="purchase-title">
              Get Your License
              <br />
              <span className="text-[hsl(43,74%,49%)]">Start Trading Today</span>
            </h2>
            <p className="text-white/60 text-base leading-relaxed mb-8 max-w-lg">
              Purchase your AI Sniper EA license PIN instantly with crypto. Once payment is confirmed,
              your unique PIN is generated automatically. Enter it in the EA and start trading XAUUSD.
            </p>

            {/* Features */}
            <div className="space-y-3 mb-8">
              {[
                "Unique license PIN generated instantly after payment",
                "Full access to AI Sniper EA v2.0 with ML pattern learning",
                "All strategy modes: Trend, Range, Breakout",
            ...
            <div className="flex items-end gap-2 mb-2">
              <span className="font-mono text-5xl font-black text-[hsl(43,74%,49%)]">
                ${price}
              </span>
              <span className="text-white/40 text-sm mb-2">USD / one-time</span>
            </div>
            <p className="text-xs text-white/40 font-mono">Paid via crypto (USDC / stablecoins)</p>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 p-8" data-testid="purchase-form">
            ...
            <div className="space-y-4">
              ...
              {error && (
                <div className="text-[hsl(348,83%,47%)] text-sm" data-testid="purchase-error">{error}</div>
              )}
              ...
              >
                ...
                {loading ? "REDIRECTING TO PAYMENT..." : `PAY $${price} WITH CRYPTO`}
              </button>
