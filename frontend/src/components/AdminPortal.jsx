...
function EmailTab({ api }) {
  ...
  const emailContent = useCallback(() => ({
    subject,
    body_html: body,
    preview_text: preview || null,
    cta_label: ctaLabel.trim() || null,
    cta_url: ctaUrl.trim() || null,
  }), [body, ctaLabel, ctaUrl, preview, subject]);
  const payload = () => ({ mode, to: to.trim(), ...emailContent(), confirm: mode === "broadcast" });

  useEffect(() => {
    if (!canCompose || (ctaUrl.trim() && !/^https?:\/\//i.test(ctaUrl.trim()))) {
      setPreviewHtml("");
      return undefined;
    }
    const timer = window.setTimeout(() => {
      ax.post(`${api}/admin/email/preview`, emailContent(), h)
        .then(r => setPreviewHtml(r.data.html || ""))
        .catch(() => setPreviewHtml(""));
    }, 350);
    return () => window.clearTimeout(timer);
  }, [api, canCompose, ctaUrl, emailContent, h]);

  const sendTest = async () => {
    if (!canCompose) { setError("Add a subject and message first."); return; }
    setTestBusy(true); setError(""); setResult(null);
    try {
      const r = await ax.post(`${api}/admin/email/test`, emailContent(), h);
  ...
  return (
    <div className="max-w-2xl space-y-5" data-testid="admin-email-tab">
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Button label (optional)">
              <Input data-testid="email-cta-label" value={ctaLabel} onChange={e => setCtaLabel(e.target.value)} placeholder="Open Command Center" maxLength={80} />
            </Field>
            <Field label="Button URL (optional)">
              <Input data-testid="email-cta-url" type="url" value={ctaUrl} onChange={e => setCtaUrl(e.target.value)} placeholder="https://xaucloud.io/command" />
            </Field>
          </div>
      ...
      <CardSection title="Recipient preview">
        <div className="mb-3 flex items-center justify-between gap-3">
          <p className="text-[11px] leading-5 text-white/40">Generated by the same template renderer used for test and broadcast sends.</p>
          <div className="flex rounded-lg bg-white/[0.04] p-1">
            {[['desktop', 'Desktop'], ['mobile', 'Mobile']].map(([value, label]) => (
              <button key={value} type="button" onClick={() => setPreviewWidth(value)}
                className={`rounded-md px-2.5 py-1 text-[10px] font-bold ${previewWidth === value ? 'bg-gold-300 text-black' : 'text-white/45'}`}>
                {label}
              </button>
            ))}
          </div>
        </div>
        <div className="overflow-auto rounded-xl border border-white/10 bg-[#eceff1] p-3 sm:p-5" data-testid="email-recipient-preview">
          {previewHtml ? (
            <iframe title="XauCloud recipient email preview" srcDoc={previewHtml} sandbox=""
              className="mx-auto block h-[620px] rounded-lg border-0 bg-white transition-[width] duration-200"
              style={{ width: previewWidth === "mobile" ? 375 : "100%", maxWidth: "100%" }} />
          ) : (
            <div className="flex h-48 items-center justify-center text-center text-[12px] text-black/45">
              Add a subject and message to preview the actual recipient email.
            </div>
          )}
        </div>
      </CardSection>
