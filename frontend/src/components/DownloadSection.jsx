...
export default function DownloadSection({ api }) {
  ...
  const version  = info?.version  || "v6.24.17";
  const edition  = info?.edition  || "INDICATOR RECOVERY PATIENCE FIX";
  const filename = info?.filename || "XAUUSD_AI_Sniper_EA_v6.24.17.mq5";
        ...
        <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-3 max-w-2xl rounded-xl border border-white/[0.07] bg-white/[0.02] px-4 py-3 text-[12px] leading-5 text-white/45">
              <span className="font-semibold text-amber-200">Gold-only v6.24.1.</span> One aligned candidate lifecycle enters fresh structure promptly, rejects genuinely consumed extensions, and preserves full-risk-or-block sizing.
            </p>
          </div>
