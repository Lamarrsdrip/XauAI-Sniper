...
export default function DownloadSection({ api }) {
  ...
  const version  = info?.version  || "v6.17.13";
  const edition  = info?.edition  || "INDICATOR ROOT-CAUSE + WATCHDOG DEDUP";
  const filename = info?.filename || "XAUUSD_AI_Sniper_EA_v6.17.13.mq5";
        ...
        <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-3 max-w-2xl rounded-xl border border-white/[0.07] bg-white/[0.02] px-4 py-3 text-[12px] leading-5 text-white/45">
              <span className="font-semibold text-amber-200">Gold-only v6.17.13.</span> Found the exact cause of a real scanning stall from the live journal: one indicator kept hitting a known, harmless MetaTrader timing quirk, but the bot was treating it as a real failure and burning 90+ seconds retrying every time. That's fixed, and the recovery log is now clean instead of spamming the same line hundreds of times.
            </p>
          </div>
