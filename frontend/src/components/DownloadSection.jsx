...
export default function DownloadSection({ api }) {
  ...
  const version  = info?.version  || "v6.17.14";
  const edition  = info?.edition  || "RISK CAP + FLEET CONSISTENCY";
  const filename = info?.filename || "XAUUSD_AI_Sniper_EA_v6.17.14.mq5";
        ...
        <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-3 max-w-2xl rounded-xl border border-white/[0.07] bg-white/[0.02] px-4 py-3 text-[12px] leading-5 text-white/45">
              <span className="font-semibold text-amber-200">Gold-only v6.17.14.</span> Raised the per-trade risk cap and loosened the spread gate after showing the exact math behind each. Also fixed a real fleet-consistency bug: an account's own unrelated prior loss could make it silently miss a valid signal that identical accounts took — that signal is now preserved and re-checked once on the next candle instead of being lost.
            </p>
          </div>
