...
export default function DownloadSection({ api }) {
  ...
  const version  = info?.version  || "v6.20.2";
  const edition  = info?.edition  || "COMMAND SAFETY + FORCE-CONTROL AUDIT";
  const filename = info?.filename || "XAUUSD_AI_Sniper_EA_v6.20.2.mq5";
        ...
        <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-3 max-w-2xl rounded-xl border border-white/[0.07] bg-white/[0.02] px-4 py-3 text-[12px] leading-5 text-white/45">
              <span className="font-semibold text-amber-200">Gold-only v6.20.2.</span> Command Center force-open now carries original signal audit data, and force-close can target one exact EA ticket with symbol and magic-number protection.
            </p>
          </div>
