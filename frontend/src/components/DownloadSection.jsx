...
export default function DownloadSection({ api }) {
  ...
  const version  = info?.version  || "v6.17.2";
  const edition  = info?.edition  || "GLOBAL VETO FIX";
  const filename = info?.filename || "XAUUSD_AI_Sniper_EA_v6.17.2.mq5";
        ...
        <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-3 max-w-2xl rounded-xl border border-white/[0.07] bg-white/[0.02] px-4 py-3 text-[12px] leading-5 text-white/45">
              <span className="font-semibold text-amber-200">Gold-only v6.17.2.</span> Runtime-proven fix: a legacy global veto was silently undoing the v6.17.0 direction fix — it zeroed out any countertrend candidate whenever the slow HTF reading disagreed, even after Adaptive Direction had already confirmed a real reversal. The veto now exempts confirmed reversals while still blocking genuinely unconfirmed countertrend trades.
            </p>
          </div>
