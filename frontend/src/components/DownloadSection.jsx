...
export default function DownloadSection({ api }) {
  ...
  const version  = info?.version  || "v6.15.0";
  const edition  = info?.edition  || "STRATEGY-LED RECONSTRUCTION";
  const filename = info?.filename || "XAUUSD_AI_Sniper_EA_v6.15.0.mq5";
        ...
        <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-3 max-w-2xl rounded-xl border border-white/[0.07] bg-white/[0.02] px-4 py-3 text-[12px] leading-5 text-white/45">
              <span className="font-semibold text-amber-200">Gold-only v6.15.0.</span> Strategy leads, AI advises: a new AI authority switch defaults away from full AI control, a hard-block on a bad setup can no longer be waved through by grade alone, and a new Adaptive Direction Engine stops trend-follow entries from firing against live short-term structure.
            </p>
          </div>
