...
export default function DownloadSection({ api }) {
  ...
  const version  = info?.version  || "v6.17.25";
  const edition  = info?.edition  || "ENTRY-PATH CONSISTENCY";
  const filename = info?.filename || "XAUUSD_AI_Sniper_EA_v6.17.25.mq5";
        ...
        <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-3 max-w-2xl rounded-xl border border-white/[0.07] bg-white/[0.02] px-4 py-3 text-[12px] leading-5 text-white/45">
              <span className="font-semibold text-amber-200">Gold-only v6.17.25.</span> Every path to a live order -- fresh signals, re-entries after a stop-out, and recovered setups -- now reads the current chart the same way before executing, and a manual override always stays a true override.
            </p>
          </div>
