...
export default function DownloadSection({ api }) {
  ...
  const version  = info?.version  || "v6.16.0";
  const edition  = info?.edition  || "DIRECTION ENGINE V2";
  const filename = info?.filename || "XAUUSD_AI_Sniper_EA_v6.16.0.mq5";
        ...
        <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-3 max-w-2xl rounded-xl border border-white/[0.07] bg-white/[0.02] px-4 py-3 text-[12px] leading-5 text-white/45">
              <span className="font-semibold text-amber-200">Gold-only v6.16.0.</span> Adaptive Direction Engine v2 reads M5+M15 structure (BOS, CHoCH, HH/HL vs LH/LL, displacement, failed breakout) to tell a real pullback apart from a breakdown, switches direction faster without banning a side for the session, and a universal pre-execution check keeps actual risk within the approved cap on every trade.
            </p>
          </div>
