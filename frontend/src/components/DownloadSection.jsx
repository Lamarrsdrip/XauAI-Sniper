...
export default function DownloadSection({ api }) {
  ...
  const version  = info?.version  || "v6.9.0";
  const edition  = info?.edition  || "No-Limit Loss-Close Firewall";
  const filename = info?.filename || "XAUUSD_AI_Sniper_EA_v6.9.0.mq5";
        ...
        <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-3 max-w-2xl rounded-xl border border-white/[0.07] bg-white/[0.02] px-4 py-3 text-[12px] leading-5 text-white/45">
              <span className="font-semibold text-amber-200">Gold-only v6.9.0.</span> Adds no-limit daily/growth-lock bypasses, a loss-close firewall for EA exit paths, and exact close-reason reporting while preserving the v6.6.0 static-review fixes.
            </p>
          </div>
