...
//|                                     v5.1.2 — Smart Profit Lock    |
//+------------------------------------------------------------------+
#property copyright "XauAI Sniper by emriz.eth"
#property link      "https://xauaisniper.com"
#property version   "5.12"
#property description "XAUUSD AI Sniper v5.1.2 — Smart Profit Lock"
#property description "  • Loose trend gate (ATR×1.0 + ranging-market carve-out) → bot trades chop, not just trends"
#property description "  • Escalating HWM giveback (25%→20%→15%→10%) → +30% day locks 80%, +75% day locks 90%"
#property description "  • Per-position ratchet (BE @+1×ATR, trail @+2×ATR) → individual winners can't roundtrip"
#property description "  • Adaptive cooldown (1L=30m, 2L=90m, 3L+=240m) → single losses don't paralyze the bot"
#property strict
...
input group "=== PROFIT GUARDIAN (v5.1.2 — escalating giveback, per-position ratchet, adaptive cooldown) ==="
input bool   InpProfitGuardian      = true;  // master switch for the entire guardian module
input double InpPG_Tier1Pct         = 30.0;  // when daily gain ≥ this %, halve risk + cap pyramids
input double InpPG_Tier2Pct         = 50.0;  // when daily gain ≥ this %, quarter risk + A+ only + no pyramids
input double InpPG_Tier3Pct         = 75.0;  // when daily gain ≥ this %, no new entries, just trail open trades
input double InpPG_HWMGivebackPct   = 25.0;  // BASE giveback % (when daily gain < 30%). Scales tighter automatically as gain grows.
input bool   InpPG_EscalatingGiveback = true;  // v5.1.2: TRUE → tighten allowed giveback as the day's gain grows (locks big runs)
input double InpPG_GivebackAt30Pct  = 20.0;  // v5.1.2: max giveback% when day gain ≥ 30% (tighter than base)
input double InpPG_GivebackAt50Pct  = 15.0;  // v5.1.2: max giveback% when day gain ≥ 50%
input double InpPG_GivebackAt75Pct  = 10.0;  // v5.1.2: max giveback% when day gain ≥ 75% (lock big runs hard)
input bool   InpPG_HTFTrendLock     = true;  // block counter-trend entries when HTF strongly opposes
input ENUM_TIMEFRAMES InpPG_HTFTrendTF = PERIOD_M30;  // HTF for trend lock — M30 default
input double InpPG_HTFTrendATR      = 1.0;   // v5.1.2: HTF price must be > EMA50 + this×ATR for "strong trend" (was 2.0 — too strict, blocked 7hr)
input bool   InpPG_ConsolidationCarveout = true;  // v5.1.2: if last 10 M30 bars are ranging, skip the trend lock (lets bot scalp chop)
input double InpPG_ConsolidationATR = 0.8;   // v5.1.2: range/ATR ratio threshold — < this means "consolidating"
input int    InpPG_PostLossCooldown = 30;    // BASE cooldown minutes after a basket loss > 5% of balance
input bool   InpPG_AdaptiveCooldown = true;  // v5.1.2: scale cooldown by consecutive losses (1L=base, 2L=3×, 3L+=8×)
input bool   InpPG_PerPositionRatchet = true; // v5.1.2: at +1×ATR profit move SL→BE; at +2×ATR trail 1×ATR behind
input double InpPG_RatchetBETrigger  = 1.0;  // v5.1.2: ATR multiples in profit before BE move
input double InpPG_RatchetTrailStart = 2.0;  // v5.1.2: ATR multiples in profit before trail kicks in
input double InpPG_RatchetTrailDist  = 1.0;  // v5.1.2: ATR distance behind price to trail SL
...
void OnTick()
{
   ...
   string pgBlock = PG_BlockReason(signal, grade);
...
void PG_UpdateHWM()
{
   if(!InpProfitGuardian || dailyStartEquity <= 0) return;
   double equity = accInfo.Equity();
   ...
   if(hwmGain > 0)
   {
      ...
      if(currentDrawback >= maxAllowedGiveback && !pg_dayHaltActive)
      {
...
string PG_BlockReason(int signal, string grade)
{
...
void PG_PerPositionRatchet()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      double newSL = curSL;
      // Stage 1: BE move
      if(profitInAtr >= InpPG_RatchetBETrigger)
      {
         double beSL = entry;
         if(ptype == POSITION_TYPE_BUY  && (curSL == 0 || curSL < beSL)) newSL = beSL;
         if(ptype == POSITION_TYPE_SELL && (curSL == 0 || curSL > beSL)) newSL = beSL;
      }
      // Stage 2: trail at InpPG_RatchetTrailDist × ATR behind price (only tightens, never loosens)
      if(profitInAtr >= InpPG_RatchetTrailStart)
      {
         double trail = atr * InpPG_RatchetTrailDist;
                       ...
                       string grade, double riskHintPct)
{
   ...
   string body = StringFormat(
      "{\"symbol\":\"%s\",\"side\":\"%s\",\"entry\":%.5f,\"sl\":%.5f,\"tp\":%.5f,\"grade\":\"%s\",\"risk_hint_pct\":%.3f}",
      symbol, side, entry, sl, tp, grade, riskHintPct);
   char pd[], res[]; string rh;
   StringToCharArray(body, pd, 0, StringLen(body));
...
void UpdateDashboard(int signal, double score, string grade)
{
   ...
   string d = "\n";
   d += "==========================================\n";
   d += " XAUAI SNIPER v5.1.0 | PROFIT GUARDIAN | ";
