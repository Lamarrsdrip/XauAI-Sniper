...
//|                                     v5.1.5 — Score floor + truth   |
//+------------------------------------------------------------------+
#property copyright "XauAI Sniper by emriz.eth"
#property link      "https://xauaisniper.com"
#property version   "5.15"
#property description "XAUUSD AI Sniper v5.1.5 — Score floor + log truth (fixes 24h+ silent idle)"
#property description "  • FIX: combined score formula no longer multiplies setupScore to death by quality factors"
#property description "         (e.g. Score:4.5 × regime 0.5 × session 0.95 = 2.0 below B threshold)."
#property description "         New floor InpScoreFloor=0.65 — combined ≥ setupScore × 0.65 always."
#property description "  • FIX: misleading log '[PASS] — PASS' actually meant REJECTED. Now '[SKIP]'"
#property description "         and explicit 'TRADE BLOCKED BECAUSE: <reason>' on every gate."
#property description "  • FIX: post-loss gradeB tightening capped at 3.0 (was 3.5) and EXPIRES after"
#property description "         InpStaleStreakHours=6 of no trades — kills the death-spiral lockout."
#property description "  • Inherits v5.1.4: giveback noise filter; v5.1.3: Profit Guardian off by default."
#property strict
...
input bool   InpAdaptiveGradeB = true;     // v5.1.5: tighten gradeB after losses, loosen after wins
input double InpAdaptiveGradeBMax = 3.0;   // v5.1.5: was 3.5 — caps the death-spiral after a losing streak
input int    InpStaleStreakHours = 6;      // v5.1.5: forget post-loss tightening after this many hours of no trades
...
void OnTick()
{
   ...
   double combinedFloor = setupScore * InpScoreFloor;
   ...
   if(InpAdaptiveGradeB && InpAdaptiveGrades && patternCount >= 20)
   {
      int winN = 0, lossN = 0;
      ...
      // v5.1.5 — escape hatch: if last trade was > InpStaleStreakHours ago,
      // drop the streak penalty so the bot resumes trading on fresh setups.
      bool stale = false;
      ...
      if(totalN >= 10 && !stale)
      {
         ...
         if(recentWR < 0.40)      dynGradeB = MathMin(InpAdaptiveGradeBMax, InpGradeB + 0.75);
         else if(recentWR > 0.60) dynGradeB = MathMax(1.8, InpGradeB - 0.50);
      }
   }
   ...
   if(signal == 0 || combinedScore < dynGradeB)
   {
      ...
      string blockReason = (signal == 0)
         ...
         : StringFormat("combined %.1f < threshold %.1f%s (raw=%.1f rq=%.2f sq=%.2f)",
                        combinedScore, dynGradeB,
                        (dynGradeB > InpGradeB) ? " [tightened post-loss]" : "",
                        combinedRaw, regimeQuality, sessionQuality);
...
void PG_UpdateHWM()
{
   ...
   //         Without this, normal price wiggle on a +0.5% day fires the brake silently.
...
string PG_BlockReason(int signal, string grade)
{
   ...
   // 1. Daily halt active — fires under InpProfitLock too (the whole point of v5.1.3)
