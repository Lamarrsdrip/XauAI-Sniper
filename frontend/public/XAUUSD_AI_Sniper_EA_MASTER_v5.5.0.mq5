...
//|                                     v5.5.0 — Equity Preservation Framework|
//+------------------------------------------------------------------+
#property copyright "XauAI Sniper by emriz.eth"
#property link      "https://xauaisniper.com"
#property version   "5.50"
#property description "XAUUSD AI Sniper v5.5.0 — EQUITY PRESERVATION FRAMEWORK"
#property description "Tiered defensive system: T1(+5%) T2(+10%) T3(+15%) T4(+20%)."
#property description "Peak-retrace gate, consecutive-loss intelligence, cluster protection."
#property description "Partial close at +1.5R on trend-aligned trades during winning phases."
#property description "Preserves profitable phases — does NOT cripple aggression."
...
input bool   InpPyramidOnAdverse= true;    // v5.3.1: re-enabled with confidence gate (only A/A+ + score≥4 can stack adverse). Tighter than v5.2.x raw add-on-anything.
...
int OnInit()
{
   ...
   Print("=== XAUAI SNIPER v5.5.0 (EQUITY PRESERVATION FRAMEWORK) READY ===");
...
void CheckPyramidOpportunity()
{
   ...
   // v5.5.0 — EPF T2+ blocks NEW pyramid adds (existing pyramids managed normally)
   if(EPF_BlockPyramidAdd())
   {
      static datetime lastEPFPyrLog = 0;
      if(TimeCurrent() - lastEPFPyrLog > 300)  // throttle to once per 5 min
      {
         Print("🛑 EPF-T", epf_tier, " PYRAMID-ADDS DISABLED (preservation mode)");
         lastEPFPyrLog = TimeCurrent();
      }
      return;
   }
   ...
   if(adverseTrigger && !trendTrigger)
   {
   ...
   string preBlock = PreTradeBlockReason(isBuy ? +1 : -1);
   ...
   if(ok)
   {
      ...
      if(CloudEnabled())
      {
         ...
         CloudPostReasoning("PYR", StringFormat("PYRAMID #%d %.2f lot @%.2f — %s",
...
// Called early in OnTick on every M5 bar boundary.
int EPF_ComputeTier()
{
   ...
   // Hysteresis: when going UP through tiers use the threshold;
   // when coming DOWN, allow 1% slack before relaxing (prevents flapping).
   ...
   // Peak-retrace path: if user gave back N% from intraday peak, bump 1 tier up.
   // (only if peak was meaningful)
   if(retraceFromPeakPct >= InpEPF_PeakRetracePct && peakGainPct >= InpEPF_PeakMinPct)
      tier = MathMax(tier, 2);   // at minimum: moderate defense
   if(retraceFromPeakPct >= InpEPF_PeakRetracePct * 1.5 && peakGainPct >= InpEPF_PeakMinPct)
      tier = MathMax(tier, 3);

   // Consecutive-loss path
   if(pg_consecutiveLosses >= InpEPF_ConsecLossWarn)
      tier = MathMax(tier, 1);
   if(pg_consecutiveLosses >= InpEPF_ConsecLossCool)
   {
      tier = MathMax(tier, 3);
      // Trigger / extend cooldown
      if(epf_cooldownUntil < TimeCurrent())
         epf_cooldownUntil = TimeCurrent() + InpEPF_CooldownMin * 60;
   }

   // Hard cooldown active → lockdown
   if(TimeCurrent() < epf_cooldownUntil) tier = 4;
...
string EPF_BlockReason(string grade, double setupScore, int signal)
{
   ...
   // T4 = full lockdown on new entries
   if(epf_tier >= 4)
      return StringFormat("EPF-T4 LOCKDOWN (consec-loss cooldown %d sec remaining)",
                          (int)(epf_cooldownUntil - TimeCurrent()));
   // T3 = preservation: A/A+ only + setupScore floor
   if(epf_tier >= 3)
   {
      if(grade != "A" && grade != "A+")
         return StringFormat("EPF-T3 PRESERVE: only A/A+ allowed (got %s)", grade);
      if(setupScore < 3.5)
         return StringFormat("EPF-T3 PRESERVE: setupScore %.1f < 3.5 floor", setupScore);
   }
   // T2 = moderate defense: A/A+ only
   if(epf_tier >= 2)
   {
      if(grade != "A" && grade != "A+")
         return StringFormat("EPF-T2 DEFENSE: only A/A+ allowed (got %s)", grade);
   }
   // T1 = mild defense: setupScore >= 3.0 (weeds out marginal B trades)
   if(epf_tier >= 1)
   {
      if(setupScore < 3.0)
         return StringFormat("EPF-T1 MILD: setupScore %.1f < 3.0 floor", setupScore);
   }
   return "";
}
...
void OnTick()
{
   ...
   double combinedFloor = setupScore * effFloor;
   ...
   // v5.1.5: cap tightening at InpAdaptiveGradeBMax (3.0 default, was 3.5) and
   //         FORGET the tightening if no trades have happened in InpStaleStreakHours
   //         hours — otherwise a 4-loss streak permanently locks the bot out
   //         because no new pattern data can arrive (death spiral).
   // v5.1.8: respect admin Bot Mode override (Conservative=on, Balanced/Aggressive=off)
   double dynGradeB = effGradeB;
   if(GetEffectiveAdaptiveGrade() && InpAdaptiveGrades && patternCount >= 20)
   {
      int winN = 0, lossN = 0;
      ...
      bool stale = false;
      if(lastTradeClose > 0 && InpStaleStreakHours > 0 &&
         TimeCurrent() - lastTradeClose > InpStaleStreakHours * 3600)
         stale = true;
      if(totalN >= 10 && !stale)
      {
         double recentWR = (double)winN / totalN;
         if(recentWR < 0.40)      dynGradeB = MathMin(InpAdaptiveGradeBMax, effGradeB + 0.75);
         else if(recentWR > 0.60) dynGradeB = MathMax(1.8, effGradeB - 0.50);
      }
   }
   ...
   string pgBlock = PG_BlockReason(signal, grade, combinedScore);
...
string PreTradeBlockReason(int signal)
{
   ...
   if(IsMomentumWeak(signal))     return "momentum slowdown (close in opposite 30% of last 3-bar range)";
...
string PG_BlockReason(int signal, string grade, double combinedScore)
{
   ...
   string preBlock = PreTradeBlockReason(signal);
   if(StringLen(preBlock) > 0)
...
void UpdateDashboard(int signal, double score, string grade)
{
   ...
   string d = "\n";
   d += "==========================================\n";
   d += " XAUAI SNIPER v5.5.0 | MODE:" + g_modeName + " | ";
