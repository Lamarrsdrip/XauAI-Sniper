...
#property description "XAUUSD AI Sniper v5.1.0 — Profit Guardian (protect intraday gains, block counter-trend stacks)"
#property description "NEW: 3-tier risk taper as daily gain grows (30%→halve, 50%→quarter+A+only, 75%→halt entries)"
#property description "NEW: HWM-giveback brake — halts trading rest of day if 25%+ of HWM gain is given back"
#property description "NEW: HTF Trend Lock — blocks counter-trend SELLs in H4 uptrends (and vice versa) unless A+"
#property description "NEW: 30-min cooldown after any basket flush > 5% of balance (anti-revenge guard)"
#property description "Inherits v5.0.0 Cloud Fanout + v4.9.7 Smart Guards (basket protect, fast-reversal, R-exits)."
#property strict
...
input group "=== PROFIT GUARDIAN (v5.1.0 — protect intraday gains, block counter-trend revenge stacks) ==="
...
input double InpPG_HWMGivebackPct   = 25.0;  // hard stop: if equity drops by this % of HWM-gain, halt for the day
input bool   InpPG_HTFTrendLock     = true;  // block counter-trend entries when H4 strongly opposes
input double InpPG_HTFTrendATR      = 1.5;   // H4 price must be > EMA50 + this×ATR for "strong trend"
input int    InpPG_PostLossCooldown = 30;    // minutes to pause after a basket loss > 5% of balance
...
void OnTick()
{
   ...
   // v5.1.0 — Profit Guardian: keep HWM updated every tick, trigger day-halt if needed
   PG_UpdateHWM();
...
void PG_UpdateHWM()
{
   ...
   if(hwmGain > 0)
   {
      double maxAllowedGiveback = hwmGain * (InpPG_HWMGivebackPct / 100.0);
      ...
      if(currentDrawback >= maxAllowedGiveback && !pg_dayHaltActive)
      {
         ...
         Print("🛡 PROFIT GUARDIAN: Day-halt triggered. HWM gain $",
               ...
               DoubleToString(InpPG_HWMGivebackPct,1), "% of gain). NO NEW ENTRIES until tomorrow.");
      }
   }
}
...
int PG_HTFTrend()
{
   ...
   // Cached every 5 min — H4 doesn't shift fast.
   static datetime lastCheck = 0; static int lastTrend = 0;
   if(TimeCurrent() - lastCheck < 300) return lastTrend;
   lastCheck = TimeCurrent();
   double ema[3], close[3], atr[3];
   int hEMA = iMA(Symbol(), PERIOD_H4, 50, 0, MODE_EMA, PRICE_CLOSE);
   int hATR = iATR(Symbol(), PERIOD_H4, 14);
   ...
   if(CopyClose(Symbol(), PERIOD_H4, 0, 3, close) <= 0) return lastTrend;
...
string PG_BlockReason(int signal, string grade)
{
   ...
   // 1. Daily halt active
   if(pg_dayHaltActive) return "PG day-halt (giveback >25% of HWM gain)";
   ...
   if(pg_pauseUntil > 0 && TimeCurrent() < pg_pauseUntil)
   {
      ...
      return StringFormat("PG cooldown (post-loss, %d:%02d remaining)", secs/60, secs%60);
   }
   ...
   if(htf == +1 && signal == -1 && !isAPlus)
      return "PG HTF lock (H4 strong UP — sells blocked, A+ only would pass)";
   if(htf == -1 && signal == +1 && !isAPlus)
      return "PG HTF lock (H4 strong DOWN — buys blocked, A+ only would pass)";
...
void PG_OnBasketLoss(double lossPct)
{
   ...
   {
      pg_pauseUntil = TimeCurrent() + InpPG_PostLossCooldown * 60;
      Print("🛡 PROFIT GUARDIAN: Post-loss cooldown ", InpPG_PostLossCooldown,
            "min triggered (basket loss ", DoubleToString(lossPct,2), "% of balance).");
   }
}
