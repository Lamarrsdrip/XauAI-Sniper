...
Author: XauAi Sniper   |   Version: 1.4.0  (persistent open_tickets + MT5 reconcile + orphan sweep)
"""
...
class WorkerAgent:
    ...
    def compute_lots(self, u: UserSession, sig: dict) -> Tuple[float, str]:
        """v1.3 STRICT MIRROR: lot size is the master's lot scaled by balance ratio.
        Returns (lots, source_label_for_logging).
          - If signal carries master_lots + master_balance: lots = (userBalance / masterBalance) * masterLots
          - Else (legacy / missing): fall back to risk-tier based calc.
        Identical balances → identical lots. Smaller user balance → proportionally smaller."""
        ...
        if master_lots > 0 and master_bal > 0:
            ratio = u.last_balance / master_bal if master_bal > 0 else 1.0
            lots = max(0.01, round(master_lots * ratio, 2))
            return min(lots, 100.0), f"strict_mirror master={master_lots:.2f}@${master_bal:.0f} user=${u.last_balance:.0f} ratio={ratio:.3f}"
        ...
        lots = max(0.01, round(risk_usd / (sl_dist * 100.0), 2))
        return min(lots, 10.0), f"legacy_fallback risk={risk_pct}% (master EA outdated — no master_lots in signal)"
