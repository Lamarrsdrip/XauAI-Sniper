...
Author: XauAi Sniper   |   Version: 1.5.1  (partial-close sync + account-size lot scaling)
"""
...
MAX_RISK_PCT_PER_TRADE = float(os.environ.get("WORKER_MAX_RISK_PCT", "10.0"))
...
class WorkerAgent:
    ...
    def mt5_order_close_partial(self, u: UserSession, ticket: int, close_percent: float) -> Tuple[float, float, float, float, str]:
        ...
        for filling in self._supported_fillings(info):
            ...
            if res is not None and res.retcode in (mt5.TRADE_RETCODE_DONE, mt5.TRADE_RETCODE_PLACED, 10010):
                ...
                return float(close_price), est_profit, float(close_vol), float(remaining), ""
            if res is not None and res.retcode == 10030:
                continue
    ...
    def compute_lots(self, u: UserSession, sig: dict) -> Tuple[float, str]:
        """v1.4.6 STRICT MIRROR + AUTO-FIT-TO-ACCOUNT:
          1. lot = (userBalance / masterBalance) × masterLots  (strict 1:1)
          2. If estimated risk > MAX_RISK_PCT% of equity → SCALE DOWN the lot
             so risk == cap, then take the trade. (Previously: SKIP. User pain:
             master fires aggressive 9-lot pyramid trades; even a bigger cloud
             account would refuse them. New: cloud always takes the trade — at
             a size that fits account risk tolerance.)
          3. ABORT only if (a) account below MIN_BALANCE_USD floor OR
                          (b) even auto-scaled lot < 0.01 broker minimum.
        ...
        if master_lots > 0 and master_bal > 0:
            ...
            scaled_note = ""
            # AUTO-SCALE: if mirrored lot exceeds cap, reduce to fit cap exactly.
            if risk_pct > MAX_RISK_PCT_PER_TRADE and sl_dist > 0:
                cap_usd = u.last_balance * MAX_RISK_PCT_PER_TRADE / 100.0
                fitted_lot = round(cap_usd / (sl_dist * XAU_USD_PER_LOT_PER_PRICE), 2)
                scaled_note = (f" | AUTO-SCALED: mirror was {lots:.2f} lots "
                               f"({risk_pct:.1f}%) → reduced to {fitted_lot:.2f} "
                               f"to fit {MAX_RISK_PCT_PER_TRADE:.1f}% cap")
                lots = fitted_lot
                est_loss_usd = lots * sl_dist * XAU_USD_PER_LOT_PER_PRICE
                risk_pct = (est_loss_usd / u.last_balance * 100.0) if u.last_balance > 0 else 999
            ...
            if lots < 0.01:
                return 0.0, (f"SKIP under-min: lot {lots:.4f} < 0.01 broker min "
                             f"(user=${u.last_balance:.0f} master=${master_bal:.0f} "
            ...
            return final_lots, (f"strict_mirror masterLots={master_lots:.2f} masterBal=${master_bal:.0f} "
                                f"userBal=${u.last_balance:.0f} ratio={ratio:.4f} "
        ...
        if lots < 0.01:
            return 0.0, (f"SKIP legacy-under-min: calculated {lots:.4f} < 0.01 "
                         f"from risk={risk_pct_cfg}% balance=${u.last_balance:.0f}")
