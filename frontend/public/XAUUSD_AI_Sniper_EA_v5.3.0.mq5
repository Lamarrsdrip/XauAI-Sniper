...
#property description "XAUUSD AI Sniper v5.3.0 — Smart-edge bundle (volatility/spread kill, RSI div, momentum slowdown, fake breakout, dynamic basket TP, ATR pyramid spacing, ML smoothing, hard daily DD)"
...
input double InpHardDailyDDPct    = 3.0;    // Hard-halt new entries when daily PnL ≤ -X% of day-start equity (separate from PG giveback)
...
input bool   InpPyramidOnAdverse= false;   // v5.3.0: DEFAULT FALSE — adverse-pyramid was the root cause of the 09:15 / 13:45 disasters (-$12k). Enable only with strong evidence of mean-reversion regime.
...
bool ManageBasket()
{
   ...
   {
      double cl[3]; int hATR2 = iATR(Symbol(), PERIOD_M5, 14);
      double atrBuf[3];
      if(CopyClose(Symbol(), PERIOD_M5, 0, 3, cl) >= 3 &&
         hATR2 != INVALID_HANDLE && CopyBuffer(hATR2, 0, 0, 3, atrBuf) >= 3)
      {
...
string VolatilityKillReason()
{
   ...
   int hATR = iATR(Symbol(), PERIOD_M5, 14);
   if(hATR == INVALID_HANDLE) return "";
   double atrBuf[51];
   if(CopyBuffer(hATR, 0, 0, 51, atrBuf) < 51) return "";
...
string SpreadKillReason()
{
   ...
   double sample[60];
   ArrayCopy(sample, g_spreadSamples, 0, 0, g_spreadCount);
   ArraySort(sample);
...
// v5.3.0 — Phase 1: Hard daily DD floor (account-size aware via dailyStartEquity).
string HardDailyDDReason()
{
   ...
   if(pnlPct <= -InpHardDailyDDPct)
      return StringFormat("hard daily DD floor (%.2f%% ≤ -%.2f%% — locked till next day)",
...
bool HasExhaustionDivergence(int signal)
{
   ...
   int hRSI = iRSI(Symbol(), PERIOD_M5, 14, PRICE_CLOSE);
   if(hRSI == INVALID_HANDLE) return false;
   double rsi[20], hi[20], lo[20], cl[20];
   ...
   if(CopyClose(Symbol(), PERIOD_M5, 0, 20, cl) < 20) return false;
...
bool PG_HTFAlignedM15H1(int signal)
{
   ...
   for(int i = 0; i < 2; i++)
   {
      int hEMA = iMA(Symbol(), tfs[i], 50, 0, MODE_EMA, PRICE_CLOSE);
      int hATR = iATR(Symbol(), tfs[i], 14);
      if(hEMA == INVALID_HANDLE || hATR == INVALID_HANDLE) return false;
      double ema[2], atr[2], close[2];
      if(CopyBuffer(hEMA, 0, 0, 2, ema) <= 0) return false;
      if(CopyBuffer(hATR, 0, 0, 2, atr) <= 0) return false;
