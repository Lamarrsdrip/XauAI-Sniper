...
//|                                     v5.1.7 — M30 context gate     |
//+------------------------------------------------------------------+
#property copyright "XauAI Sniper by emriz.eth"
#property link      "https://xauaisniper.com"
#property version   "5.17"
#property description "XAUUSD AI Sniper v5.1.7 — Faster context gate (M30 default, was H4)"
#property description "  • Context-gate HTF default M30 — H4 was 4hr stale on gold's pace, blocking valid trends."
#property description "  • Configurable via InpContextTF input — set to PERIOD_H4 to restore old behavior."
#property description "  • Inherits v5.1.6 aggressive defaults (no post-loss tightening) + v5.1.5 score floor."
#property strict
...
bool ContextGateAllows(int signal, double atr)
{
   ...
   if(InpUseH4Bias)
   {
      ...
      if(h4F > 0 && h4S > 0)
      {
         ...
         {
            string tfName = EnumToString(InpContextTF);
            if(signal == 1 && !h4Up)
            {
               PrintFormat("⛔ CONTEXT-GATE: BUY blocked — %s EMA50 < EMA200 (bearish HTF bias). Don't fight the trend.", tfName);
               return false;
            }
            if(signal == -1 && !h4Down)
            {
               PrintFormat("⛔ CONTEXT-GATE: SELL blocked — %s EMA50 > EMA200 (bullish HTF bias). Don't fight the trend.", tfName);
               return false;
            }
         }
      }
...
int OnInit()
{
   ...
   Print("=== XAUAI SNIPER v5.1.7 (M30 CONTEXT) READY ===");
...
void UpdateDashboard(int signal, double score, string grade)
{
   ...
   string d = "\n";
   d += "==========================================\n";
   d += " XAUAI SNIPER v5.1.7 | AGGRESSIVE | ";
