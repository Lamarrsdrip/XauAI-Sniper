...
#property description "XAUUSD AI Sniper v5.3.1 — Adaptive guards (soft DD mode + adverse-pyramid confidence gate + high-grade breathing room)"
...
input double InpHardDailyDDPct    = 6.0;    // v5.3.1: HARD halt only at extreme DD (was 3% — too restrictive). Soft-mode kicks in earlier via InpSoftDDPct.
input double InpSoftDDPct         = 2.5;    // v5.3.1: SOFT mode trigger. At -2.5% day PnL: A/A+ only + 0.7× lots + tighter ML. Still trades.
input double InpSoftDDLotMulti    = 0.7;    // v5.3.1: lot multiplier while in soft DD mode (0.7 = 30% reduction)
...
void CheckPyramidOpportunity()
{
   ...
   {
      bool isHighGrade = (g_lastEntryGrade == "A" || StringFind(g_lastEntryGrade, "A+") >= 0);
      if(!isHighGrade)
      {
...
string PG_BlockReason(int signal, string grade, double combinedScore)
{
   ...
   if(IsSoftDDMode())
   {
      bool isAGrade = (grade == "A" || StringFind(grade, "A+") >= 0);
      if(!isAGrade)
