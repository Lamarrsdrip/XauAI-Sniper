...
#property description "XAUUSD AI Sniper v5.2.2 — Pyramid adds also fan-out to cloud (full 1:1 stack mirror)"
...
{
   // v5.2.1 — Startup cooldown is the FIRST gate, before anything else.
   // Even with PG disabled, we never want a blind shot right after EA init.
   string startupBlock = StartupCooldownReason();
   if(StringLen(startupBlock) > 0)
      return "Trade blocked due to startup cooldown — " + startupBlock;
