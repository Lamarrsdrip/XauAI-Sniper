...
EA_ROOT = ROOT / "XAUUSD_AI_Sniper_EA_v6.12.0.mq5"
...
def test_v6412_version_identity_is_single_source_and_backend_matches_root():
    ...
    assert root == backend
    assert '#property version   "6.120"' in root
    assert '#define XAUAI_EA_VERSION "v6.12.0"' in root
    assert '#define XAUAI_EA_VERSION_NUM "6.12.0"' in root
