...
def test_backend_download_metadata_header_reports_current_release():
    ...
    assert f"v{match.group(1)}" == "v6.4.20"
    assert "Full Build Integrity Audit" in match.group(2)
    edition = match.group(2).strip().rstrip("|").strip()
    assert edition == "Full Build Integrity Audit + TTM Compile Fix"
    assert '#define XAUAI_EA_VERSION "v6.4.20"' in ea
    assert '#define XAUAI_BUILD_HASH "v6420-build-integrity-audit-20260701"' in ea
