...
// httpCode: the network call's return code. -1 means the network call itself
...
// Market Edition (Claude XauCloud) Task 2: XAU_LeaseRequestOnline() and XAU_LeaseSurrenderOnline()
// (cloud /api/cloud/lease/{request-endpoint}, /api/cloud/lease/surrender) deleted outright -- zero
// call sites anywhere in this include or the main EA (leases are provisioned out-of-band; the EA
// only ever consumes a pre-issued lease file from local disk via XAU_LeaseLoadFromDisk, unchanged).
// Also moot in this build: the cross-instance reservation claim (mq5 XAU_ClaimDirectionReservation)
// now always succeeds locally, so the offline-lease fallback branch that would have consulted these
// is itself unreachable -- see mq5 XAU_CanOpenDirection.
...
{
   // Market Edition (Claude XauCloud) Task 2: cloud /api/cloud/lease/reconcile upload removed.
   // No WebRequest call is made. Identical to this function's own documented on-failure behavior --
   // a failed/skipped upload leaves the local queue file untouched, nothing is lost, caller retries
   // later. Moot in this build regardless: the offline-lease authorize path that would ever populate
   // this queue is itself unreachable now (see XAU_ClaimDirectionReservation in the main EA file).
   return false;
}
