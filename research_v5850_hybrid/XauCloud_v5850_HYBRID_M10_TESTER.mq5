...
//|                     XauCloud_v5.8.50_ENGINE_CURRENT_POLICY_M10    |
//|      RESEARCH CANDIDATE B -- v5.8.50 signal engine (UNCHANGED) +   |
//|      ported current-bot permanent blocks/risk/lot/SL/exits,        |
//|      evaluated on closed M10 bars instead of M5. Base: v5.8.50      |
//|      commit de2984c. research/v5850-engine-current-policy-hybrid.  |
//|      Not for live/VPS deployment -- research only.                 |
//+------------------------------------------------------------------+
#property copyright "XauAI Sniper by emriz.eth"
#property link      "https://xauaisniper.com"
#property version   "5.99"
#property description "RESEARCH CANDIDATE B: v5.8.50 engine + current-bot policy, M10 cadence"
...
//+------------------------------------------------------------------+
input group "=== LICENSE ==="
input string InpLicensePIN     = "ASE-TEST-0001";
...
//+------------------------------------------------------------------+
//| PORTED OWNER POLICY MODULE (research/v5850-engine-current-policy-  |
//| hybrid, 2026-07-31). v5.8.50's own ScoreSetups/candidate-generation |
//| engine below this block is UNCHANGED -- this module only adds the  |
//| current production bot's permanent category blocks and owner       |
//| location EXCELLENT/LATE hard block, ported by BEHAVIOR (not by     |
//| importing the Adaptive Transition / TRANSITION_WATCH machinery     |
//| those blocks live inside on the current bot -- that machinery is   |
//| explicitly excluded from this experiment). Location quality here   |
//| is a minimal, v5.8.50-native proxy built from existing EMA/ATR      |
//| evidence and the existing lastClose tracker, not from the current  |
//| bot's Adaptive Transition Engine.                                   |
//+------------------------------------------------------------------+
enum ENUM_XAU_HYBRID_LOCATION
{
   HLOC_EXCELLENT,      // very close to value (EMA) -- current bot's evidence found this
                        // an unconditional automated-entry hard block (60-day replay:
                        // allowing it -$3.4k/PF0.84, blocking it +$12.3k/PF1.39)
   HLOC_GOOD,
   HLOC_ACCEPTABLE,
   HLOC_LATE,           // far extended from value -- also hard-blocked by owner policy
   HLOC_EXTREME,        // even further extended -- not part of the ported block scope,
                        // kept distinct only for logging, per "do not broaden" instruction
   HLOC_RESET_PENDING   // this direction's continuation just closed/paused very recently,
                        // not yet reconfirmed -- v5.8.50-native proxy via lastClose
};
input int InpHybridResetPendingWindowSec = 900; // how recent a same-direction close counts as RESET_PENDING

double XAU_HybridExtensionATR(double atr)
{
   if(atr <= 0.0) return 0.0;
   double emaF = bufEMAFast[1];
   double close1 = iClose(Symbol(), PERIOD_M10, 1);
   if(emaF <= 0.0 || close1 <= 0.0) return 0.0;
   return MathAbs(close1 - emaF) / atr;
}

ENUM_XAU_HYBRID_LOCATION XAU_HybridBucketLocation(int dir, double atr)
{
   if(lastClose.valid && lastClose.dir == dir &&
      (TimeCurrent() - lastClose.closeTime) < InpHybridResetPendingWindowSec)
      return HLOC_RESET_PENDING;
   double ext = XAU_HybridExtensionATR(atr);
   if(ext >= 2.2) return HLOC_EXTREME;
   if(ext >= 1.6) return HLOC_LATE;
   if(ext <= 0.5) return HLOC_EXCELLENT;
   if(ext <= 1.0) return HLOC_GOOD;
   return HLOC_ACCEPTABLE;
}

string XAU_HybridLocationName(ENUM_XAU_HYBRID_LOCATION loc)
{
   switch(loc)
   {
      case HLOC_EXCELLENT:     return "LOCATION_EXCELLENT";
      case HLOC_GOOD:          return "LOCATION_GOOD";
      case HLOC_ACCEPTABLE:    return "LOCATION_ACCEPTABLE";
      case HLOC_LATE:          return "LOCATION_LATE";
      case HLOC_EXTREME:       return "LOCATION_EXTREME";
      case HLOC_RESET_PENDING: return "LOCATION_RESET_PENDING";
   }
   return "LOCATION_UNKNOWN";
}

bool XAU_HybridIsAsiaSession()
{
   // Same 0-8 server/broker-hour window this file's own GetSessionQuality()
   // already uses to identify Asia -- not a new convention.
   MqlDateTime dt; TimeCurrent(dt);
   return (dt.hour >= 0 && dt.hour < 8);
}

bool XAU_HybridReversalSetup(string setupName)
{
   // v5.8.50's own mean-reversion/counter-trend setups (trade against the
   // extended/extreme price rather than with structure) -- TREND_PULLBACK,
   // BREAKOUT, SQUEEZE_RELEASE, LONDON_FIX_PIN, ASIA_BREAKOUT are trend-
   // following/continuation and are NOT classified as reversal here, per
   // "do not block normal Grade B trend-following or pullback trades".
   return (setupName == "RANGE_REVERSAL" || setupName == "RSI_EXTREME" || setupName == "MULTI_EXTREME");
}

// SINGLE shared gate -- called identically at candidate-acceptance time and
// again immediately before order-send with fresh live values, so the two
// enforcement points can never drift apart (same function, same logic, no
// separate re-implementation to fall out of sync).
bool XAU_HybridPermanentGateCheck(int dir, string setupName, string grade, double atr, string &blockReason)
{
   blockReason = "";
   string canonicalGrade = grade;
   StringToUpper(canonicalGrade);
   StringTrimLeft(canonicalGrade); StringTrimRight(canonicalGrade);

   bool isAsia = XAU_HybridIsAsiaSession();
   ENUM_XAU_HYBRID_LOCATION loc = XAU_HybridBucketLocation(dir, atr);

   // PERMANENT OWNER POLICY 1: during Asia, only A+ candidates are allowed.
   if(isAsia && canonicalGrade != "A+")
   {
      blockReason = "PERM_BLOCK_ASIA_NON_A_PLUS";
      return false;
   }

   // PERMANENT OWNER POLICY 2: A+ at RESET_PENDING blocked everywhere.
   if(canonicalGrade == "A+" && loc == HLOC_RESET_PENDING)
   {
      blockReason = "PERM_BLOCK_A_PLUS_RESET_PENDING";
      return false;
   }

   if(canonicalGrade == "B")
   {
      // PERMANENT OWNER POLICY 3: Grade B reversal/opposite-discovery blocked.
      if(XAU_HybridReversalSetup(setupName))
      {
         blockReason = "PERM_BLOCK_GRADE_B_REVERSAL";
         return false;
      }
      // PERMANENT OWNER POLICY 4: Grade B RESET_PENDING blocked.
      if(loc == HLOC_RESET_PENDING)
      {
         blockReason = "PERM_BLOCK_RESET_PENDING_GRADE_B";
         return false;
      }
   }

   // PERMANENT OWNER POLICY 5: LOCATION_EXCELLENT / LOCATION_LATE are an
   // absolute, unconditional automated-entry hard block (ported verbatim in
   // scope -- not broadened to GOOD/ACCEPTABLE/EXTREME/RESET_PENDING, which
   // are governed by the rules above or not blocked at all).
   if(loc == HLOC_EXCELLENT || loc == HLOC_LATE)
   {
      blockReason = (loc == HLOC_EXCELLENT) ? "OWNER_LOCATION_EXCELLENT_BLOCK" : "OWNER_LOCATION_LATE_BLOCK";
      return false;
   }
...
void XAU_HybridLogBlock(string phase, int dir, string setupName, string grade, double atr, string reason)
{
   PrintFormat("HYBRID_PERMANENT_BLOCK | phase=%s | direction=%s | setup=%s | grade=%s | session=%s | location=%s | reason=%s",
...
double XAU_HybridRiskPerLotForDistance(double dist)
{
   ...
   if(ref <= 0.0) ref = iClose(Symbol(), PERIOD_M10, 1);
...
bool ContextGateAllows(int signal, double atr)
{
   ...
   if(InpUseSRFilter && InpSRLookback >= 20 && atr > 0)
   {
      ...
      {
         double h = iHigh(Symbol(), PERIOD_M10, i);
         double l = iLow(Symbol(),  PERIOD_M10, i);
         bool isSwingHigh = true;
         bool isSwingLow  = true;
         for(int j = 1; j <= 3; j++)
         {
            if(iHigh(Symbol(), PERIOD_M10, i-j) >= h || iHigh(Symbol(), PERIOD_M10, i+j) >= h) isSwingHigh = false;
            if(iLow(Symbol(),  PERIOD_M10, i-j) <= l || iLow(Symbol(),  PERIOD_M10, i+j) <= l)  isSwingLow  = false;
...
int OnInit()
{
   ...
   datetime barOpens[1]; if(CopyTime(Symbol(), PERIOD_M10, 0, 1, barOpens) > 0)
   ...
   // M5 indicators
   hEMAFast  = iMA(Symbol(), PERIOD_M10, InpEMAFast, 0, MODE_EMA, PRICE_CLOSE);
   hEMASlow  = iMA(Symbol(), PERIOD_M10, InpEMASlow, 0, MODE_EMA, PRICE_CLOSE);
   hRSI      = iRSI(Symbol(), PERIOD_M10, InpRSIPeriod, PRICE_CLOSE);
   hATR      = iATR(Symbol(), PERIOD_M10, InpATRPeriod);
   hBBUpper  = iBands(Symbol(), PERIOD_M10, 20, 0, 2.0, PRICE_CLOSE);
   hBBLower  = hBBUpper;
   hBBMid    = hBBUpper;
   ...
   hStoch    = iStochastic(Symbol(), PERIOD_M10, 14, 3, 3, MODE_SMA, STO_LOWHIGH);
   ...
   int barsAvail = Bars(sym, PERIOD_M10);
...
bool CopyEntryBuffer(int handle, int buffer, int start, int count, double &target[], string label)
{
   ...
   if(TimeCurrent() - g_lastIndicatorFailLog >= 60)
   {
      Print("SCAN BUFFER WAIT: ", g_lastSkipReason,
            " | bars M5=", Bars(Symbol(), PERIOD_M10),
...
double DetectRegime()
{
   ...
   if(bufBBUpper[5] > 0 && bufBBLower[5] > 0)
      prevBBWidth = (bufBBUpper[5] - bufBBLower[5]) / iClose(Symbol(), PERIOD_M10, 5) * 100;
...
string BuildSignature(int dir, string setupName)
{
   ...
   double atr = bufATR[1];
   double mom = iClose(Symbol(), PERIOD_M10, 1) - iClose(Symbol(), PERIOD_M10, 5);
...
void UpdateAsiaRange()
{
   ...
   {
      double h = iHigh(Symbol(), PERIOD_M10, 1);
      double l = iLow(Symbol(),  PERIOD_M10, 1);
...
int ScoreSetups(double &score, string &setupName)
{
   ...
   double bbU = bufBBUpper[1], bbL = bufBBLower[1], bbM = bufBBMid[1];
   double close1 = iClose(Symbol(), PERIOD_M10, 1);
   double close2 = iClose(Symbol(), PERIOD_M10, 2);
   double open1 = iOpen(Symbol(), PERIOD_M10, 1);
   double high1 = iHigh(Symbol(), PERIOD_M10, 1);
   double low1 = iLow(Symbol(), PERIOD_M10, 1);
   double h1F = bufEMAFast_H1[1], h1S = bufEMASlow_H1[1];
      ...
      (currentRegime == REGIME_BREAKOUT_UP || currentRegime == REGIME_BREAKOUT_DOWN))
   {
      ...
      long vol = iVolume(Symbol(), PERIOD_M10, 1);
      long avgVol = (iVolume(Symbol(), PERIOD_M10, 2) + iVolume(Symbol(), PERIOD_M10, 3)) / 2;
   ...
   // === SETUP 4: SQUEEZE RELEASE ===
   {
      ...
      double prevW = 0;
      if(bufBBUpper[10] > 0) prevW = (bufBBUpper[10] - bufBBLower[10]) / iClose(Symbol(), PERIOD_M10, 10) * 100;
   ...
   // === SETUP 6: LONDON FIX PIN ===
   {
      ...
      if(isLondonFix)
      {
         ...
         double move = close1 - iClose(Symbol(), PERIOD_M10, 4);
   ...
   if(InpAsiaRangeBreakout && asiaRangeLocked && asiaRangeHigh > 0 && asiaRangeLow > 0)
   {
      ...
      if(dt.hour >= 7 && dt.hour < 17)
      {
         ...
         if(rangeSize > atr * 1.5)
         {
            ...
            double hi3 = MathMax(iHigh(Symbol(), PERIOD_M10, 2), iHigh(Symbol(), PERIOD_M10, 3));
            double lo3 = MathMin(iLow(Symbol(),  PERIOD_M10, 2), iLow(Symbol(),  PERIOD_M10, 3));
            ...
            {
               // Volume confirmation
               long v1 = iVolume(Symbol(), PERIOD_M10, 1);
               long vAvg = (iVolume(Symbol(), PERIOD_M10, 2) + iVolume(Symbol(), PERIOD_M10, 3) + iVolume(Symbol(), PERIOD_M10, 4)) / 3;
...
bool SmartGuardStrongTrendRetest(int signal, double setupScore, double combinedScore)
{
   ...
   double mom = (iClose(Symbol(), PERIOD_M10, 1) - iClose(Symbol(), PERIOD_M10, 5)) / atr;
...
double PyramidMomentumATR(int dir, double atr)
{
   ...
   double c1 = iClose(Symbol(), PERIOD_M10, 1);
   double c4 = iClose(Symbol(), PERIOD_M10, 4);
...
void CheckPyramidOpportunity()
{
   ...
   {
      double h = iHigh(Symbol(), PERIOD_M10, k);
      double l = iLow(Symbol(), PERIOD_M10, k);
   ...
   // PORTED OWNER POLICY -- pyramid adds have their own separate order-send
   // call site (not routed through OpenTrade()), so they get their own gate
   // call using the same shared XAU_HybridPermanentGateCheck function -- same
   // logic as both OpenTrade enforcement points, applied here so the pyramid
   // path cannot bypass the permanent blocks / owner location block.
   {
      int pyrDir = isBuy ? 1 : -1;
      string pyrGrade = (StringLen(g_lastEntryGrade) > 0) ? g_lastEntryGrade : "B";
      string hybridBlockReason3 = "";
      if(!XAU_HybridPermanentGateCheck(pyrDir, why, pyrGrade, bufATR[1], hybridBlockReason3))
      {
         XAU_HybridLogBlock("PYRAMID", pyrDir, why, pyrGrade, bufATR[1], hybridBlockReason3);
         Print("PYRAMID BLOCKED (owner policy): ", hybridBlockReason3, " | ", why);
         return;
      }
   }
...
void OnTick()
{
   ...
   datetime barOpens[1];
   datetime curBar = 0;
   int barCopied = CopyTime(Symbol(), PERIOD_M10, 0, 1, barOpens);
   if(barCopied > 0) curBar = barOpens[0];
   if(curBar <= 0)  curBar = iTime(Symbol(), PERIOD_M10, 0);
   ...
   PrintFormat("SIGNAL_TF=M10 | closedBarTime=%s | candidateCycle=%d", TimeToString(curBar, TIME_DATE|TIME_MINUTES), (int)curBar);
   ...
   double firstSeenPx = signal > 0 ? SymbolInfoDouble(Symbol(), SYMBOL_ASK)
                                   : SymbolInfoDouble(Symbol(), SYMBOL_BID);
   if(firstSeenPx <= 0.0) firstSeenPx = iClose(Symbol(), PERIOD_M10, 1);
   ...
   if(grade == "A+" && InpUseAI && StringLen(InpServerURL) >= 10)
   {
      int aiResult = GetAIAnalysis(bufEMAFast[1], bufEMASlow[1], bufRSI[1], bufATR[1],
         iClose(Symbol(), PERIOD_M10, 1),    // closed bar — consistent with setup scoring
         bufEMAFast_H1[1] > bufEMASlow_H1[1] ? "BULL" : "BEAR", spread,
         setupName, RegimeName(), signature,
         ArraySize(bufStochK) >= 2 ? bufStochK[1] : 50.0,
         iClose(Symbol(), PERIOD_M10, 1) - iClose(Symbol(), PERIOD_M10, 5));
   ...
   // PORTED OWNER POLICY -- ENFORCEMENT POINT 1 of 2 (candidate acceptance).
   // See XAU_HybridPermanentGateCheck definition for full policy detail.
   {
      string hybridBlockReason = "";
      if(!XAU_HybridPermanentGateCheck(signal, setupName, grade, bufATR[1], hybridBlockReason))
      {
         XAU_HybridLogBlock("CANDIDATE_ACCEPTANCE", signal, setupName, grade, bufATR[1], hybridBlockReason);
         XAU_RememberBlockedSignal(signal, setupName, grade, setupScore, combinedScore, hybridBlockReason);
         CloudPostReasoning("BLOCK", hybridBlockReason, RegimeName(), setupName,
                            setupScore, combinedScore, "HYBRID-PERMANENT-BLOCK", signal);
         UpdateDashboard(0, combinedScore, hybridBlockReason);
         lastDashSignal = 0; lastDashScore = combinedScore; lastDashGrade = hybridBlockReason;
         return;
      }
   }
...
void OpenTrade(int signal, double atr, string reason, double sizeMulti)
{
   ...
   // PORTED OWNER POLICY -- ENFORCEMENT POINT 2 of 2 (immediately before
   // order-send, fresh recompute). OpenTrade() is the single execution
   // function used by the primary signal path, RE_ENTRY and pyramid paths
   // (see call sites), so this one insertion covers all of them -- no
   // separate/alternate execution function exists in this file to bypass it.
   // reason encodes "setupName [grade]" from the primary path; RE_ENTRY/other
   // callers that don't supply a bracketed grade are treated as Grade B (the
   // conservative default) rather than skipping the check.
   {
      string hybridSetupName = reason;
      string hybridGrade = "B";
      int braOpen = StringFind(reason, " [");
      if(braOpen >= 0 && StringGetCharacter(reason, StringLen(reason)-1) == ']')
      {
         hybridSetupName = StringSubstr(reason, 0, braOpen);
         hybridGrade = StringSubstr(reason, braOpen+2, StringLen(reason)-braOpen-3);
      }
      string hybridBlockReason2 = "";
      if(!XAU_HybridPermanentGateCheck(signal, hybridSetupName, hybridGrade, atr, hybridBlockReason2))
      {
         XAU_HybridLogBlock("FINAL_EXECUTION", signal, hybridSetupName, hybridGrade, atr, hybridBlockReason2);
         Print("ORDER-SEND BLOCKED (final gate): ", hybridBlockReason2, " | ", reason);
         return;
      }
   }
...
bool ManageBasket()
{
   ...
   {
      double cl[3]; int hATR_dbtp = iATR(Symbol(), PERIOD_M10, 14);
      double atrBuf[3];
      if(CopyClose(Symbol(), PERIOD_M10, 0, 3, cl) >= 3 &&
...
void CleanStructureLevels(int lookback, double &swingLow, double &swingHigh)
{
   ...
   {
      double hk = iHigh(Symbol(), PERIOD_M10, k);
      double lk = iLow(Symbol(), PERIOD_M10, k);
...
int CleanStructureBreakBars(bool isBuy, double swingLow, double swingHigh, double structBuf)
{
   ...
   {
      double c = iClose(Symbol(), PERIOD_M10, k);
      if(c <= 0) break;
      bool broke = false;
                                 ...
                                 int minsOpen, double lotsOpen)
{
   ...
   if(rMult >= trailStartR && momentumConfirmed)
   {
      ...
      if(lb < 3) lb = 3;
      double anchor = isBuy ? iHigh(Symbol(), PERIOD_M10, iHighest(Symbol(), PERIOD_M10, MODE_HIGH, lb, 0))
                            : iLow(Symbol(),  PERIOD_M10, iLowest(Symbol(),  PERIOD_M10, MODE_LOW,  lb, 0));
...
void ManagePositions()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      double close2EA = iClose(Symbol(), PERIOD_M10, 2);
      ...
      if(profit >= EffProfitTakeMin()
         ...
         && earnedFirstR)
      {
         double close2alt = iClose(Symbol(), PERIOD_M10, 2);
...
string StartupCooldownReason()
{
   ...
   {
      datetime barOpens[1];
      if(CopyTime(Symbol(), PERIOD_M10, 0, 1, barOpens) <= 0)
...
string VolatilityKillReason(int signal, string setupName)
{
   ...
   int hVK = iATR(Symbol(), PERIOD_M10, 14);
...
bool HasExhaustionDivergence(int signal)
{
   ...
   int hRSI_div = iRSI(Symbol(), PERIOD_M10, 14, PRICE_CLOSE);
   ...
   if(CopyHigh(Symbol(), PERIOD_M10, 1, 20, hi) < 20) { IndicatorRelease(hRSI_div); return false; }
   if(CopyLow (Symbol(), PERIOD_M10, 1, 20, lo) < 20) { IndicatorRelease(hRSI_div); return false; }
...
bool IsMomentumWeak(int signal)
{
   ...
   if(CopyHigh(Symbol(), PERIOD_M10, 1, 3, hi) < 3) return false;
   if(CopyLow (Symbol(), PERIOD_M10, 1, 3, lo) < 3) return false;
   if(CopyClose(Symbol(), PERIOD_M10, 1, 3, cl) < 3) return false;
...
double XAU_DirectionalExtensionATR(int signal, int bars, double atr, double &resetATR)
{
   ...
   double hi = iHigh(Symbol(), PERIOD_M10, 1);
   double lo = iLow(Symbol(), PERIOD_M10, 1);
...
bool IsXAUConfirmedBreakoutContinuation(int signal, string setupName)
{
   ...
   if(CopyOpen (Symbol(), PERIOD_M10, 1, 3, o) < 3) return false;
   if(CopyHigh (Symbol(), PERIOD_M10, 1, 3, h) < 3) return false;
   if(CopyLow  (Symbol(), PERIOD_M10, 1, 3, l) < 3) return false;
   if(CopyClose(Symbol(), PERIOD_M10, 1, 3, c) < 3) return false;
...
bool IsFakeBreakout(int signal)
{
   ...
   if(CopyHigh(Symbol(), PERIOD_M10, 2, N, hi) < N) return false;
   if(CopyLow (Symbol(), PERIOD_M10, 2, N, lo) < N) return false;
   if(CopyClose(Symbol(), PERIOD_M10, 1, 2, cl) < 2) return false;
...
double XAU_SessionVWAP(int lookbackBars)
{
   ...
   {
      datetime bt = iTime(Symbol(), PERIOD_M10, i);
      if(bt <= 0) break;
      ...
      double h = iHigh(Symbol(), PERIOD_M10, i);
      double l = iLow(Symbol(), PERIOD_M10, i);
      double c = iClose(Symbol(), PERIOD_M10, i);
      long   v = iVolume(Symbol(), PERIOD_M10, i);
...
void XAU_RunStartupIntelligenceSync()
{
   ...
   int barsM5 = Bars(Symbol(), PERIOD_M10);
   ...
   double bid = SymbolInfoDouble(Symbol(), SYMBOL_BID);
   double ask = SymbolInfoDouble(Symbol(), SYMBOL_ASK);
   double mid = (bid > 0.0 && ask > 0.0) ? (bid + ask) * 0.5 : iClose(Symbol(), PERIOD_M10, 0);
                              ...
                              double setupScore, double combinedScore)
{
   ...
   if(CopyOpen(Symbol(), PERIOD_M10, 1, 1, o) < 1) return;
   if(CopyHigh(Symbol(), PERIOD_M10, 1, 1, h) < 1) return;
   if(CopyLow(Symbol(), PERIOD_M10, 1, 1, l) < 1) return;
   if(CopyClose(Symbol(), PERIOD_M10, 1, 1, c) < 1) return;
   double bid = SymbolInfoDouble(Symbol(), SYMBOL_BID);
   double ask = SymbolInfoDouble(Symbol(), SYMBOL_ASK);
                               ...
                               string reason)
{
   ...
   double px = (signal > 0) ? SymbolInfoDouble(Symbol(), SYMBOL_ASK)
                            : SymbolInfoDouble(Symbol(), SYMBOL_BID);
   if(px <= 0.0) px = iClose(Symbol(), PERIOD_M10, 1);
   if(px <= 0.0) return;
                         ...
                         string &grade, double &lotMulti, string &reason)
{
   ...
   double close1 = iClose(Symbol(), PERIOD_M10, 1);
   double close2 = iClose(Symbol(), PERIOD_M10, 2);
   double close4 = iClose(Symbol(), PERIOD_M10, 4);
   double open1  = iOpen(Symbol(), PERIOD_M10, 1);
   double high1  = iHigh(Symbol(), PERIOD_M10, 1);
   double low1   = iLow(Symbol(), PERIOD_M10, 1);
   ...
   for(int i = 1; i <= 6; i++)
   {
      hi6 = MathMax(hi6, iHigh(Symbol(), PERIOD_M10, i));
      lo6 = MathMin(lo6, iLow(Symbol(), PERIOD_M10, i));
   }
                        ...
                        double &lotMulti, string &reason, bool logDecision)
{
   ...
   int m5  = TFDirectionByEMA(signal, PERIOD_M10,  tfThreshold, m5Why);
...
void PG_PerPositionRatchet()
{
   ...
   double atrBuf[1];
   int hATR = iATR(Symbol(), PERIOD_M10, InpATRPeriod);
