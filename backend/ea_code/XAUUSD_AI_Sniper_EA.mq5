   ...
   if(!InpBacktestMode && StringLen(InpServerURL) >= 10)
   {
      ...
      if(wrTest == 200)
      {
         ...
         if(calCode == 200 && ArraySize(calResult) > 10)
         {
            ...
            // Parse multipliers — simple string scan (no full JSON parser needed)
            // Expected keys: "0-49", "50-64", "65-79", "80-100"
            auto ParseBand = [](string json, string key) -> double {
               string pattern = "\"" + key + "\":";
               ...
               while(pos < StringLen(json))
               {
                  ...
                  if((ch >= '0' && ch <= '9') || ch == '.' || ch == '-') { numStr += ShortToString(ch); pos++; }
                  else break;
               }
               double v = StringToDouble(numStr);
               return (v > 0.0) ? MathMax(0.70, MathMin(1.30, v)) : 1.0;
            };
            // Lambdas not supported in MQL5 — use inline helper approach:
            // Instead we do it directly with a helper macro-style approach:
