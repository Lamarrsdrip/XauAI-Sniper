//+------------------------------------------------------------------+
//| XauCloudLeaseCryptoTestEA.mq5                                     |
//| Same self-test as XauCloudLeaseCryptoTest.mq5, packaged as a     |
//| minimal Expert Advisor so it can be run non-interactively through |
//| Strategy Tester (this repo's established pattern for automated    |
//| MQL5 execution) and its Print() output captured from the Tester   |
//| log/journal file.                                                  |
//+------------------------------------------------------------------+
#property strict
...
void RunSha256KnownAnswerTests()
{
   uchar empty[];
   ArrayResize(empty, 0);
   uchar h1[];
   XAU_Sha256Compute(empty, h1);
   string h1hex = "";
   for(int i = 0; i < 32; i++) h1hex += StringFormat("%02x", h1[i]);
   CheckTrue("sha256-empty-string", h1hex == "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855", true);

   uchar abc[3]; abc[0]='a'; abc[1]='b'; abc[2]='c';
   uchar h2[];
   XAU_Sha256Compute(abc, h2);
   string h2hex = "";
   for(int i = 0; i < 32; i++) h2hex += StringFormat("%02x", h2[i]);
   CheckTrue("sha256-abc-nist-vector", h2hex == "ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad", true);
}
...
int OnInit()
{
   ...
   return(INIT_FAILED); // deliberately abort init -- this EA never trades, it only self-tests
}
